...
(\Cref{fig:results1}A--C, \Cref{fig:pvs}A) and includes major surface
arteries of radius \discuss{0.5--1.4 mm} (\Cref{fig:pvs}C). Imposing
...
maximum velocity of 0.55 $\mu$m/s (\Cref{fig:pvs}D, H). On the other
hand, traveling waves of arterial wall motion (\Cref{fig:pvs}B), such
...
complete description of this mathematical model, see
\Cref{sec:more_maths}. The initial concentrations in the
three-dimensional domain $\Omega$ and networks $\Lambda_a, \Lambda_v$
are set to zero for all simulation scenarios.
...
\Cref{sec:more_maths}). We do not model bulk flow within the brain
