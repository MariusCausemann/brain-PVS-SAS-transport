...
coefficients, and $\bm u$ and $\hat{u}$ are fluid velocities, all
defined in the respective compartments; $P = 2 \pi R_2$ is the
perimeter and $A = \pi (R_2^2 - R_1^2)$ the area of the PVS
cross-sections, $\delta_{\Gamma}$ is the Dirac delta function
...
of the mathematical model, see \Cref{sec:more_maths}.
...
We model molecular clearance, via e.g.~meningeal lymphatics, along the
upper, outer (arachnoid) boundary (\Cref{fig:intracranial_domains}),
proportional to the concentration in the SAS with a rate constant
$\beta_{\rm exit}$. We set a no-flux condition at the end nodes of the
perivascular networks, not allowing for direct molecular clearance
there.
...
\subsection{Mathematical definitions and further model specifications}
