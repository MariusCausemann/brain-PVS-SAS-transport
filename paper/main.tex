...
nodes from $\Lambda^j$ ($j = 1, 2, 3$) that have degree 1 (are leaf or
root nodes) or degree $\geq 3$ (are true bifurcation points), and each
path between points with degree $\not = 2$ in $\Lambda^j$ represented
by a single edge in $\mathcal{T}^j$. 
