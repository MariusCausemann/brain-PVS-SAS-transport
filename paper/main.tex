...
\item
  ...
  R_1)/R_1$. And so, for $R_a = n R_1$, then $A_e/A = n-1$.}. Note that
  representing the surface PVS as annular cylinders is a
  simplification, and several studies emphasize the assymmetric nature
  of the PVS shape~\cite{mestre2018flow, tithof2019hydraulic,
    vinje2021brain, raicevic2023sizes}.
  \begin{itemize}
  \item
...
\item
  Smets et al~\cite{smets2024perivascular} study the size of perivascular spaces surrounding pial arteries and veins in mice. They report ...
  \end{itemize}
\item
  Cerebrospinal fluid flow characteristics: magnitudes and directionality?
  \begin{itemize}
  \item
    In terms of movement, Bedussi et al~\cite{bedussi2018paravascular} found a net movement of particles in the PVS in the antegrade (same direction as blood flow) direction with an average velocity of $17 \pm 2\mu$m/s, and a mean amplitude of the pulsatile motion of $14 \pm 2 \mu$m.
\item Mestre et al~\cite{mestre2018flow} report a mean (averaged over time and space) flow speed of $18.7 \mu$m/s (root-mean-square velocity?) with net transport in the antegrade direction. 
...
For the perivascular network, the boundary nodes are collected into $\partial \Lambda$. We assume no tracer outflow on all boundary nodes and precribe a homogenous Neumann condition: 
\begin{equation}
...
\begin{table}
  ...
  \begin{tabular}{ll|llllll|ll}
    ...
    \midrule
    A & PVS sheaths & $R_2 = x R_1$ & $0, 0.5 \xi, \xi, 2 \xi, 1000 \xi \approx \infty$ & $10^{-4}$ mm$^2$/s\cite{hornkjol2022csf} & $\alpha D^{\rm gad}$\cite{sykova2008diffusion, valnes2020apparent} & $\hat{u}_{\rm prod}$ & $\mathbf{u}_{\rm prod}$ & 0 \\ 
  ...
  \caption{Overview of models (-- denotes as immediately above). Each
    ...
    radius $R_1$ and outer radius $R_2$ and such that $R_2 = \alpha
    R_1$ with $x \approx 2.0-3.0$ based on~\cite{bedussi2018paravascular, mestre2018flow, raicevic2023sizes}. Model A and its variations aims to
...
\begin{align}
\bm V_h  & = \{ \bm v  \in H(\mathrm{div}; \Omega_{\mathrm{CSF}}): \; \bm v \vert_{E} \in BDM_2 (E),  \; E \in \mathcal{E}_h; \;  \bm v \cdot \bm n = 0  \} \\ 
S_h  & = \{q \in L^2(\Omega):  \;  q \vert_E \in \mathbb{P}^1(E),  \; E \in \mathcal{E}_h \}. 
\end{align}
Here, $H(\mathrm{div};\Omega)$ is the space of $L^2$ vector fields with $L^2$ divergence,  $BDM_1$ is the Brezzi-Douglas--Marini element \cite{brezzi1987mixed} of order 1, and  $\bm n$ (resp. $\bm \tau$) is the unit outward normal (resp. tangent) vector.  
...
} \int_{F} \mu \{\nabla u\} \bm n_F [\bm v_t] 
\\ - \sum_{
F \in \mathcal{F}_{h,c} \cup \mathcal{F}_{\Gamma} 
} \int_{F} \mu \{\nabla v\} \bm n_F [\bm u_t]
+ \sum_{
F \in \mathcal{F}_{h,c} \cup \mathcal{F}_{\Gamma} 
} \frac{\sigma}{h} \int_{F} [\bm u_t ] [\bm v_t].   
\end{multline}
...
\end{equation}
The parameter $\gamma_{D,F}$ is the harmoninc mean of the diffusion coeffiecnt given by 
$$
...
\end{multline}
For the 1D concentration $\hat c$, we recall that $\hat c = \hat c_i $ on each segement of the network $\Lambda^i$.  We enforce continuity of the concentration on the bifurcation points and conservation of fluxes. We let $\mathcal{V}$ denote the set of all vertices in $\Lambda$ and $ 
...
Considering the mean tracer concentrations in the CSF, parenchyma and arterial and venous PVS domain over the first 24\,h after injection, we observe negligable changes with both mesh and time refinement (\cref{fig:mesh_convergence_concentrations} and 
