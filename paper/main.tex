...
\subsection{Estimating net perivascular flow induced by peristaltic pumping}
...
is described by a periodic travelling wave of relative amplitude
...
(see also \Cref{tab:pvs:parameters}). These geometric parameters and
...
%For each of the subnetworks $\Lambda^j$, via the minimal tree
%representation $\mathcal{T}^j$, we compute the time-averaged flow rate
%in each branch $\Lambda^k$ of $\mathcal{T}^j$
