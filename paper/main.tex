...
and the associated domain is denoted by $\Lambda_v$. Finally, we label
...
Moreover, we model the pia membrane as a semi-permeable membrane with
permeability cofficient $\beta_{\rm pia}$. For a complete description
of this mathematical model, see \Cref{sec:more_maths}.
...
To also account for the pulsatile flow of CSF in the SAS and
ventricular system with each cardiac (and respiratory) cycle, we
consider its dispersive effects as a contribution to enhanced
molecular diffusion~\cite{taylor1953dispersion, watson1983diffusion,
  keith2019dispersion, ray2021quantitative,
  troyetsky2021dispersion}. In $\Omega_{\rm CSF}$, we compute a
spatially-varying dispersion factor $R$ by combining a computational
model of cardiac-induced CSF flow at peak arterial dilation (systole)
with theoretical estimates for shear-augmented (Taylor) dispersion
\cite{taylor1953dispersion, watson1983diffusion}.
...
\subsection*{Notation and left-overs}
...
We denote the different parts of the outer boundary of $\Omega_{\rm
  CSF}$ with $\Gamma_{\mathrm{skull}}$ facing the dura and skull and
$\Gamma_{\rm SSAS}$ representing the lower interface towards the
spinal subarachnoid space (SSAS). $\Gamma_{\mathrm{skull}}$ is further
subdivided into its lower and upper parts: $\Gamma_{\mathrm{skull^l}}$
and $\Gamma_{\mathrm{skull^u}}$ (compare \Cref{fig:concept}). The
boundary of the spinal cord is denoted by $\Gamma_{\rm{SC}}$, which is
given by $\Gamma_{\rm{SC}} = \partial \Omega_{\rm PAR} \setminus
\partial \Omega_{\rm CSF}$ .
...
\subsection{Mathematical model definitions and further model specifications}
...
In this section, we describe the mathematical model~\eqref{eq:multi_transport} and associated conditions in further detail.
