...
\begin{abstract}
  ...
  human brain health, with alterations often linked to disease and
  functional impairment, and with distinct opportunities for
  personalized diagnostics and treatment. However, our understanding
  of these mechanisms and their interplay remains incomplete, in part
  due to the complexity of integrating insights across scales, between
  species, and from different modalities. In this work, we combine
  mixed-dimensional mathematical modelling, multi-modal magnetic
  resonance images, and high performance computing to construct an
  in-silico model of molecular transport within the human intracranial
  space. The model predicts the spatiotemporal spreading of a solute
  within an image-derived representation of the subarachnoid space,
  ventricular system and brain parenchyma. A key feature is the
  explicit representation of surface perivascular spaces (PVSs). To
  demonstrate, we simulate a series of scenarios relating to
  intrathecal tracer injections, including exploring the balance
  between cerebrospinal fluid (CSF) production and intracranial
  pulsatility, the role of perivascular fluid flow, and structural
  properties of the PVS. This open computational framework thus allows
  for evaluating the integrated effect of different transport
  mechanisms, bridging from the vascular mesoscale to the organ-level
  macroscale and across time scales of seconds to hours, in
...
Our key contribution is thus an in-silico modelling framework that allows for
evaluating the integrated effect of different proposed transport
mechanisms, bridging from the vascular mesoscale to the organ-level
macroscale and across time scales of seconds to days, in
...
this computational framework via a series of model scenarios targeting
key questions relating to molecular movement within the human brain
environment. In particular, we explore how the balance between CSF
production and intracranial pulsatility can shape large-scale features
of the tracer distribution. We examine how perivascular fluid flow
induced by cardiac pulsations or low-frequency vasomotion can be
...
properties, such as the absence of a structural compartmentalization
of the PVS or enlarged PVSs, can impact perivascular transport and
intracranial tracer spread. The openly available technology
platform\cite{ZENODO} allows for transferring, integrating, and
examining insights from clinical, experimental, and theoretical
studies, and provides a starting-point for future in-silico studies of
...
\begin{figure}
    ...
    %\includegraphics[width=1\textwidth]{figures/figure1.png}
     \caption{
     \textbf{An integrative computational framework for intracranial solute transport.}}
     \label{fig:results1}
\end{figure}
...
(\fixme{\Cref{fig:results1}}). As the drivers and modes of
...
debate\cite{smith2019going, proulx2021cerebrospinal,
  ...
  kipnis2025resolving}, our target is to introduce a computational
...
To this end, we introduce the following modelling choices
(\fixme{\Cref{fig:results1}}, see also Methods). We define
\fixme{three} solute concentration fields, each varying in space and
time, one in the 3D domains and one in each of the PVS networks, and
assume that the solute can cross between the 3D and 1D domains via
semi-permeable membranes. We assume that the solute will diffuse
within all compartments, with diffusivity depending on the effective
properties of the relevant medium\cite{sykova2008diffusion}. We assume
that flow in the CSF spaces will substantially affect solute
transport, and we account for two main mechanisms
(\fixme{\Cref{fig:results1}}): (i) convection via a (small) net flow
of CSF resulting from production in the choroid plexus with CSF efflux
across the upper convexity\cite{hornkjol2022csf}, and (ii) dispersive
effects resulting from the pulsatile flow of
CSF~\cite{sweetman2011cerebrospinal, vinje2019respiratory,
  ray2021quantitative}. In the surface periarterial spaces, we account
for convection via net fluid flow induced by peristaltic
pumping~\cite{mestre2018flow, gjerde2023directional}
...
$\fixme{X}$ mL/day (\Cref{fig:pvs}D,F).
...
Intrathecal administration of gadolinium-based contrast agents in
...
brain~\cite{ringstad2017glymphatic, ringstad2018brain,
  eide2024functional, van2024human, kipnis2025resolving}. To simulate
such a contrast-enhanced MRI protocol~\cite{ringstad2017glymphatic,
  ringstad2018brain,
  eide2024functional}(\fixme{\Cref{fig:intrathecal}C}), we now apply
our computational framework to predict the spreading of $0.5$ mmol
...
(\fixme{\Cref{fig:intrathecal}D--E}). To begin with, we consider a
...
After one hour, the in-silico tracer moves upwards in the SAS
...
(\fixme{\Cref{fig:intrathecal}F}). While the mean concentrations in these
compartments drop soon after peaking, the concentration in the
...
\item
  \fixme{We present an integrated model of transport. We do not
    ...
    SAS, and parenchyam are connected very little data in the
