...
(\Cref{fig:results1}A--B). We introduce two solute concentration
fields, both varying in space and time, one in the 3D domains and one
in the PVS networks, and assume that the solute can cross between
these compartments through semi-permeable membranes. As the drivers
and modes of intracranial transport are under substantial uncertainty
and debate\cite{smith2019going, proulx2021cerebrospinal,
  bohr2022glymphatic, hladky2022glymphatic, betsholtz2024advances,
  kipnis2025resolving}, the target is to introduce a computational
...
seconds to hours, in subject-specific geometries. To this end, we
introduce the following modelling assumptions. We assume that the
solute will diffuse within all compartments, with diffusivity
...
two main mechanisms: (i) convection via a (small) net flow of CSF
resulting from production in the choroid plexus with CSF efflux across
the upper convexity\cite{hornkjol2022csf}, and (ii) dispersive effects
resulting from the pulsatile flow of CSF induced by the cardiac and
respiratory cycles\cite{sweetman2011cerebrospinal,
  vinje2019respiratory, ray2021quantitative}. Moreover, in the surface
...
also define a set of auxiliary CSF and PVS flow models. The
