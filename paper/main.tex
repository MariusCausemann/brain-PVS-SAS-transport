...
with their own set of simplifying assumptions. Regarding lack of
coupling, we first note that our computational model for net flow in
the surface periarterial networks is fully decoupled from (i.e.~does
not inform and is not informed by) the model for flow in the CSF
spaces induced by CSF production. Moreover, these estimates of net
flow in the PVS are based on the simplifying assumptions that human
surface PVS follow the major surface vessels, are CSF-filled but
otherwise open regions of width proportional to the vessel radius,
have fixed outer boundaries that are not permeable to fluid exchange,
and experience uniform travelling waves of vascular wall motion with
specific characteristics. Thus, in particular, we represent a total
flow of \fixme{10.5--X} mL/day in the periarterial network without
explicitly modelling how this volume enters or exits the network and
otherwise interacts with its surroundings. This modelling choice is
based on the currently incomplete knowledge of the underlying
physiology, the mathematical and computational complexity associated
with coupled mixed-dimensional fluid flow models especially the
auxiliary modelling choices to be made regarding interaction at the
network end nodes, and the lack of complete vascular network data.
...
SAS. Our approach has not been validated against direct
spatio-temporal numerical simulations of solute transport in the CSF
spaces in the presence of pulsatile fluid flow nor against clinical
flow measurements; these points should be considered in future
work. We do note that high-resolution MRI measurements by Hirschler et
al.~\cite{hirschler2025region} revealed that CSF mobility in regions
...
values computed in our model. As a result of these limitations and the
lack of direct validation data, the simulations scenarios presented
here should be viewed as demonstrations of the applicability of the
modelling framework rather than validated computational models.
...
interstitial fluid flow. To compensate, we have focused on reporting
detailed predictions in the CSF spaces and surface PVS after not more
than 24 hours. In particular, we note that, we observe tracer
accumulation at the perivascular network ends after 12 hours, similar
to that previously observed for
...
transport parameters within the human brain from contrast-enhanced MRI
data, we have found that molecular transport within the parenchyma is
...
cortical and subcortical white matter vasculature, 
