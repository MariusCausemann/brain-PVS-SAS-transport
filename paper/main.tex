...
We use the theoretical framework previously introduced by Gjerde et
...
frequency $\omega$ acting normal to the vessel wall. The wave number
$k = 2 \pi / \lambda$ by definition. Each PVS segment $\Lambda_i$ has
length $L_i$ with wave-relative length $\ell_i = k L$, inner baseline
radius $r_{o_i}'$, outer baseline radius $r_{a_i}'$, and
outer-to-inner ratio $\beta_i = r_{e_i}'/r_{o_i}'$. These geometric
parameters and the assumption of annular cylindrical PVS segments
yield a hydraulic resistance $\mathcal{R}_{o, i}$, and the additional
parameter $\Delta_i$, see \cite{gjerde2023directional} for the
complete definitions. The network has weights $\gamma_i =
r_{o_i}'^2/r_{o_0}'^2$ where $r_{o_1}'$ is the radius of the root
branch (see below)..
\begin{itemize}
\item
  Definition of subnetworks
\item
  Definition of reduced bifurcating trees
\item
