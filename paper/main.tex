...
(\Cref{fig:pvs}C). The corresponding contribution to the
periarterial flow velocity is defined for each segment $\Lambda_i$ as
$\langle Q'_i \rangle/A_i$ where $A_i = \pi ({R_2^i}^2 - {R_1^i}^2)$ is the
...
$\varepsilon = 1\%$ yielding $\hat{u}_a^c$ (\Cref{fig:pvs}D), and
very low-frequency \emph{vasomotion} with $f = 0.1$Hz, $\lambda =
0.02$m, and $\varepsilon = 10\%$ yielding $\hat{u}_a^v$
(\Cref{fig:pvs}E). No flow induced by peristalsis is considered for
