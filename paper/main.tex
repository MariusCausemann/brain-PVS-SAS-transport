...
\begin{abstract}
  The mechanisms of intracranial solute transport are fundamental to human brain health, with alterations often linked to disease and functional impairment, and with distinct opportunities for personalized diagnostics and treatment. However, our understanding of these mechanisms and their interplay remains incomplete, in part due to the complexity of integrating insights across scales, between species, and from different modalities. In this work, we combine mixed-dimensional mathematical modelling, multi-modal magnetic resonance images, and high performance computing to construct an in-silico model of molecular transport within the human intracranial space over a time course of up to 24 hours. This model predicts the spatiotemporal spreading of a solute within an image-derived geometric representation of the subarachnoid space, ventricular system and brain parenchyma. A key feature is the explicit representation of networks of surface perivascular spaces (PVSs). To demonstrate the potential of this computational framework, we simulate a series of scenarios relating to intrathecal tracer injections, including exploring the balance between cerebrospinal fluid (CSF) production and intracranial pulsatility, the role of perivascular fluid flow, and structural properties of the PVS. \mer{This openly available technology platform thus provides an opportunity to integrate separate observations on intracranial pulsatility, vascular dynamics, CSF production and efflux, as well as diffusion in neuropil, with patient-specific geometries.}
\end{abstract}
\begin{document}
...
respiratory pulsatility in the CSF spaces, convective fluid flow
driven by CSF production in the CSF spaces and peristaltic pumping in
the PVSs, as well as solute exchange and clearance across
semi-permeable membranes. Our key contribution is the in-silico
representation of perivascular solute transport via topologically
one-dimensional transport equations interacting with transport in the
three-dimensional intracranial compartments. This concept provides a
missing link to bridge mesoscale phenomena -- such as perivascular
pulsations occuring every few seconds -- with macroscale observations
of tissue-level tracer distribution over hours to days. Preliminary
comparisons indicate that the in-silico predictions align well with
the range of tracer distribution patterns reported in
contrast-enhanced MRI studies~\cite{ringstad2017glymphatic,
  ringstad2018brain, watts2019measuring, eide2024functional}. 

% Main findings and outlook
We illustrate the potential range of this computational framework via
a series of model scenarios targeting key questions relating to
molecular movement within the human brain environment. In particular,
we explore how the balance between CSF production and intracranial
...
such as the absence of a structural compartmentalization of the PVS or
enlarged PVSs, can impact perivascular transport and intracranial
tracer spread. This openly available technology platform\cite{ZENODO}
allows for transferring, integrating, and examining insights from
clinical, experimental, and theoretical studies, and provides a
starting-point for future in-silico studies of brain solute transport.
 
\section*{Results}

\fixme{We present a computational framework first, and then
  demonstrate it by simulating a contrast-enhanced MRI protocol,
  including a series of model variations.}
...
(\Cref{fig:results1}). As the drivers and modes of intracranial
transport are under substantial uncertainty and
...
framework that allows for evaluating the integrated effect of
different proposed transport mechanisms, bridging from the vascular
mesoscale to the organ-level macroscale and across time scales of
seconds to days, in subject-specific geometries.

To this end, we introduce the following modelling choices
(\Cref{fig:results1}, see also Methods). We define \fixme{three}
solute concentration fields, each varying in space and time, one in
the 3D domains and one in each of the PVS networks, and assume that
the solute can cross between the 3D and 1D domains via semi-permeable
...
of the relevant medium\cite{sykova2008diffusion}. We assume that flow
in the CSF spaces will substantially affect solute transport, and we
account for two main mechanisms (\Cref{fig:results1}): (i) convection
via a (small) net flow of CSF resulting from production in the choroid
plexus with CSF efflux across the upper
convexity\cite{hornkjol2022csf}, and (ii) dispersive effects resulting
from the pulsatile flow of CSF~\cite{sweetman2011cerebrospinal,
  vinje2019respiratory, ray2021quantitative}. In the surface
periarterial spaces, we account for convection via net fluid flow
induced by peristaltic pumping~\cite{mestre2018flow,
  gjerde2023directional} (\Cref{fig:pvs}).  Specifically, we introduce
the geometrically-reduced representation of the PVSs to bridge spatial
scales, and model pulsatility via dispersion in the CSF and
time-averaged PVS flow to bridge temporal scales. Mathematically, the
solute transport model is represented by a mixed-dimensional system of
coupled time-dependent partial differential
equations\cite{masri2024modelling}, which we solve numerically with
high accuracy using a mass-conserving finite element scheme.

To inform the convective velocity fields and dispersion factors
entering as coefficients in the transport equations, we define a set
of auxiliary CSF and PVS flow models (\Cref{fig:results1},
\Cref{fig:pvs}). Beginning with flow field driven by CSF production,
we impose a CSF inflow of $400$ mL/day into the lateral ventricles
(\Cref{fig:results1}), yielding a maximum flow velocity of
$1.85\,$mm/s (in the aqueduct) and a mean velocity in the SAS of
$2.6$\textmu m/s (\Cref{fig:results1}). To estimate the dispersive
effect of pulsatile CSF flow, we employ the theory of shear-augmented
dispersion together with computational fluid dynamics to determine
...
cisterna magna, but has little effect ($R < 1$) in most of the SAS
(\Cref{fig:results1}). The respiratory dispersive mixing reaches a
factor of up to 320 due the lower respiratory frequency of $0.25\,$Hz
(\Cref{fig:results1}).

The periarterial network extends from the internal carotid arteries
...
(\Cref{fig:pvs}). Due to imaging limitations, the network primarily
...
peristalsis in perivascular networks~\cite{gjerde2023directional} (see
Methods), we estimate that the cardiac pulse wave alone, traveling at
a frequency of 1 Hz with a wavelength of 2.0 m and a 1\% wall
displacement~\cite{jung2021novel}, will induce mainly antegrade PVS
flow with an average net velocity of 0.92 \textmu m/s while reaching
up to 7.31 \textmu m/s near larger, ventral vessels such as the MCA
(\Cref{fig:pvs}D,E), and a total flow through the periarterial network
of $\fixme{10.5}$ mL/day. The same theory predicts that strong
ultraslow vasomotions, if traveling antegrade at 0.1Hz with a
wavelength of 0.02 m and a 10\% displacement~\cite{broggini2024long},
will induce both retrograde and antegrade net PVS flow with an average
velocity of 13.05 \textmu m/s and maximum velocity 54.44 \textmu m/s,
and a total flow of $\fixme{X}$ mL/day (\Cref{fig:pvs}D,F).

The computational framework is implemented in the FEniCS finite
element software \cite{alnaes2015fenics, kuchta2020assembly}, and all
...
such a contrast-enhanced MRI protocol~\cite{ringstad2017glymphatic,
  ringstad2018brain, eide2024functional}(\Cref{fig:intrathecal}C), we
apply our computational framework to predict the spreading of $0.5$
...
(\Cref{fig:intrathecal}D--E). To begin with, we consider a reasonably
conservative set of flow patterns: modelling the dispersive effects of
pulsatile CSF flow induced by the cardiac and respiratory
...
(\Cref{fig:intrathecal}F). While the mean concentrations in these
...
\subsection*{Can variations in CSF convection and dispersion shape these tracer distribution patterns?} 

\begin{figure}%[h!]
\centering \includegraphics[width =
  0.8\textwidth]{figures/figure2.png}
...
differ substantially between subjects and in pathological conditions,
conditions also associated with alterations in the pulsatile flow of
CSF~\cite{ringstad2018brain, eide2021direction, eide2021impaired,
  eide2022altered}. Cardiac and respiratory pulsatility clearly also
varies from subject to subject and between physiological
states. Motivated by these observations, we asked whether and how the
tracer distribution patterns predicted by our computational framework
would be affected by changes in CSF production or pulsatility.
%
...
respiratory pulsations, which changed the dispersion factors by
0.25$\times$ and 4$\times$, respectively. Note that this behaviour
directly reflects the nature of our CSF model model: Stokes flow is
linear in its inflow boundary conditions, and the diffusion
...
ambient cistern (\Cref{fig:results1}): without sufficient diffusion, the
tracer is unable to cross into the posterior SAS. Indeed, in the model
variation with higher dispersion, the tracer moves through the
quadrigeminal cistern and further upwards into the longitudinal
fissure, also spreading into the cerebellum (\Cref{fig:csf}J).

\subsection*{\fixme{Perivascular flow shapes and accelerates molecular transport}}
...
pulse wave or other vasomotion~\cite{vanveluw2020vasomotion,
  ...
  broggini2024long,hauglund2025norepinephrine}, can induce net
...
network~\cite{gjerde2023directional}.
...
artery after 48 minutes (\Cref{fig:high_low_pvs}A), defined by their first-time arrival (FTA), the time at which the concentration first exceeds $0.1$ mmol/l. At all upstream locations along the middle cerebral arteries
(MCAs) and anterior cerebral artery (ACA), we found substantially
reduced FTAs with higher PVS flow (\Cref{fig:high_low_pvs}B), up to 2.5 hours
...
indicates the directionality of tracer movement -- it first arrives
in the PVS and subsequently spreads into the tissue and SAS
(\Cref{fig:high_low_pvs}C, D), and especially so with higher PVS flow. These
...
(\Cref{fig:high_low_pvs}E). Moreover, differences in tracer transport along the
PVS translate to altered distribution  patterns on the whole-organ
scale. For instance, after 4-6 hours, the faster-moving tracer in the
PVS is clearly visible in the space adjacent to the PVS of the ACA, MCA, and other arteries (\Cref{fig:high_low_pvs}G). In contrast,
there are no clear signs of early appearance of tracer surrounding the PVS in
the baseline model (\Cref{fig:high_low_pvs}F).

A natural question is whether early appearance of tracer in the PVS
could be the result of increased dispersion rather than net flow in
the PVS\cite{asgari2016glymphatic,sharp2019dispersion,bojarskaite2023sleep,asgari2016glymphatic,troyetsky2021dispersion}. Interestingly,
even increasing the dispersion factor by $100 \times$ in the PVS,
induced only minor changes in the global spreading rate compared to
the baseline (\Cref{fig:high_low_pvs}F, H), thus indicating a negative answer
to this question.
...
\subsection*{Enlarged PVSs in the SAS delay periarterial and intracranial tracer spread}
...
perivascular and intracranial tracer spread. To address this
...
dilation on PVS flow velocities and tracer distribution patterns
...
net CSF velocity reduces, which in turn alters the tracer spread
...
tracer \mer{amounts} in the parenchyma at 6 and 12 h, while the mean PVS
...
transport and clearance in human intracranial spaces, enabling
...
macroscale tracer distribution patterns, with e.g.~ventricular tracer
reflux after intrathecal injection in the absence of CSF production,
and preference towards anterior brain regions with reduced
...
the surface PVSs results in earlier appearance of tracer around major
...
barriers. Finally, dilated PVS in the SAS will cause a substantial
reduction in cardiac- and vasomotion-driven flow velocities, which in
turn impedes perivascular transport.

%% Comparison with others

% Tracer distribution predictions
While tracer spread of the brain and surrounding CSF spaces
...
fewer reports of \mer{contrast imaging} in human subjects over 0--24h. We
...
distribution pattern and a concentration peak in the SAS of $\approx$0.5
mmol/l (relative to a total amount of 1.0 mmol) occuring after 10 to 15 hours. 
Our in-silico tracer distribution patterns agree with these observations, though with an earlier rather than later peak in the CSF spaces (2 hours after first tracer appearance at the craniocervical junction). In particular, we note that the substantial individual variation observed clinically is comparable to the variation in in-silico distribution patterns associated with reduced CSF production or reduced intracranial pulsatility. We find that about 20\% of the tracer reaching the cranium enters the parenchyma, which aligns with the reported peak concentration values of $0.5\,$mmol/l in the SAS and $0.1\,$mmol/l in the parenchyma reported by Watts et al~\cite{watts2019measuring}.

Importantly, our simulation results indicate that the tracer
distribution observed clinically relies on the presence of one or more
...
(\Cref{fig:high_low_pvs}H). To illustrate the need for a stronger advective
...
here, with earlier arrival times along the PVSs, clear spread into
adjacent spaces, and accelerated tracer spread
...
transport patterns. While we, at baseline, account for the dispersion
...
in-silico predictions and contrast-enhanced MRI for a number of subjects
...
$R_{\rm CSF, 0}$. In model variations with no CSF prodution, we set
...
predictions of tracer distribution and clearance against contrast-enhanced MRI
