...
\subsection*{Can perivascular flow shape and accelerate molecular transport?}
\label{sec:pvs_flow_results}

% FIXME: MER: This was the old one. Much better isn't it? 
...
\fixme{Traveling waves of arterial wall motion (\fixme{\Cref{fig:pvs}C}),
...
network~\cite{gjerde2023directional}. Our periarterial network extends
...
of radius 0.5--1.4 mm (\Cref{fig:pvs}). Due to imaging limitations,
...
network~\cite{gjerde2023directional} (see Methods), we estimate that
...
ventral vessels such as the MCA (\Cref{fig:pvs}D,E), and a total flow
...
antegrade net perivascular flow with an average velocity of 13.05 \textmu m/s
and maximum velocity 54.44 \textmu m/s, and a total flow of
$\fixme{X}$ mL/day (\Cref{fig:pvs}D,F). \fixme{Add 1-2 sentence about decoupled flow modelling here.}}


The PVSs are recognized across species as critical pathways for solute
transport in and around the brain, and thus as potential targets for
enhancing brain solute transport and metabolic waste clearance. However,
whether CSF flows more rapidly in human PVSs stands as an open
question~\cite{bohr2022glymphatic, van2024caa,
