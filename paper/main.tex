...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Results}
%% \mer{Update: ~2800 words and 6 subsections - around 400-500 words each?}
%% \begin{enumerate}
%% \item
...
%% \item
%%   ``The role'' of PVS sheaths and permeability - does functional separation entail structural separation? %Different molecular sizes?
%% \item
...
%% %% \item
%% %%   Clearance from the parenchyma: $c_0 = 1.0$. Different substances? Save for different paper. 
%% \end{enumerate}
...
%% We then simulated the intracranial distribution and evolution of a
%% $0.5$ mmol intrathecal injection of Gadobutrol after the appearance of
%% the solute at the craniocervical junction
%% (\Cref{fig:results1}D).
...
% but the relationship is not clear/well-defined.
...
networks~\cite{gjerde2023diretional}, we estimate that the cardiac
...
A natural question is whether early PVS enrichment could also be
caused by increased dispersion rather than net flow in the PVS. Adding
an effective dispersion of $100 \times$ in the PVS only induced minor
changes in the global spreading rate compared to the baseline
(\Cref{fig:pvs}J, O).
\begin{figure}
    \centering
...
Human and rodent observations indicate that tracers concentrate in perivascular spaces surrounding the pial and subarachnoid vasculature \cite{zhang1990interrelationships,zhang1992directional,eide2024functional, mestre2018flow}. However, it remains unclear whether such enrichment patterns necessitate a structural barrier, such as a semi-permeable membrane, or if they could result from enhanced flow or mixing (\Cref{fig:compartmentalization}A). Here, we investigate how the permeability of the interface between the cerebrospinal fluid (CSF) and the PVS affects tracer enrichment around the major arterial trunks (\Cref{fig:compartmentalization}B).

We employ the high PVS velocity model from the previous section and compare a high and low permeability model, corresponding to functional and structural compartmentalization, respectively. The low permeability model has a permeability of $3.8 \cdot 10^{-7}\,$m/consistent with previous estimates for the endfoot sheath surrounding penetrating arterioles \cite{koch2023estimates}, which we consider a lower bound for the permeability between CSF and PVS. For the high permeability model, we increase the permeability by a factor of 100.


At the basal artery (BA), tracer appears soon after injection in the CSF surrounding the BA in both models \Cref{fig:compartmentalization}C and E).
In the high permeability model, tracer quickly crosses into the PVS, resulting in similar concentrations in the PVS and surrounding CSF. In contrast, the low permeability model exhibits substantially lower perivascular tracer enrichment, with a maximum gradient of 39\,mmol/l approximately two hours post-injection.
...
For the middle cerebral arteries (MCA-R and MCA-L), tracer appears first in the PVS in both models, with higher concentrations in the high permeability model (up to 20-25 mmol/l peak concentration). In the high permeability model, tracer leaks out of the PVS and spreads in the CSF pocket. With a small delay (see \Cref{fig:compartmentalization}F for peak time difference), more tracer appears via the CSF pathway.
...
Summarizing these observations (\Cref{fig:compartmentalization}F), we find that both first-time arrival and peak time are not substantially affected by the PVS-CSF permeability, indicating that fast transport along the PVS might not hinge upon a structural compartmentalization of the PVS. However, the concentration difference between the PVS and its surrounding CSF quickly decreases with higher permeability, as does the time lag between the peaks in these domains. This implies a smooth transition between the CSF and PVS rather than a sharp concentration gradient.
