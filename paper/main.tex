...
spinal compartment (\Cref{fig:concepts}). \mer{MER: I put this here, keep
  it here. How about the ventricles?}


\begin{figure}
\caption{\mer{MER: Concept figure illustrating the geometr-y/ies in some detail. Several panels}. Illustration of the location of pial vasculature and subarachnoid space. (B) Conceptual illustration of the vascular, perivascular, SAS and cortex domains (cross-section and lateral view).}
\label{fig:concept}
\end{figure}

\subsection*{Multi-dimensional intracranial transport equations}

We consider a comprehensive time-dependent convection-diffusion model~\cite{masri2023modelling} of a solute concentration in a network representing the surrounding perivascular spaces of the pial vasculature and in the surrounding three-dimensional SAS-brain domain (Fig.~\ref{fig:concept}). More specifically, we solve for the concentrations $c$  (of the solute in the fluid space~\cite{sykova2008diffusion}) defined over the SAS-brain domain $\Omega$ and $\hat c$ in a 1D network of centerlines $\Lambda$ capturing in an averaged sense the solute transport in the perivascular spaces such that the following equations hold
\begin{subequations}
\begin{alignat}{2}
 ...
 \end{alignat}
\end{subequations}
Here $\phi$ is the fluid volume fraction or porosity of the brain (where $\phi \ll 1$), of the SAS (where $\phi = 1$) and of the perivascular space (where $0 \ll \phi \leq 1$), $D$ is the effective diffusion coefficient of the relevant solute in the respective media~\cite{sykova2008diffusion} which takes different values over the SAS and over the tisssue, $\bm u$ and $\hat u$ are velocity fields in $\Omega$ and in $\Lambda$ respectively, $\xi$ is a transfer coefficient across the outer perivascular boundary, $P$ and $A$ are the perimeter and area of the perivascular spaces which vary along the centerline network, respectively, and $f$ and $\hat{f}$ represents given sources of solute in $\Omega$ and $\Lambda$, respectively.  The term involving $\delta_\Gamma$ is a source term concentrated on the outer perivascular boundary and models the interaction between the 3D--domain and the perivascular spaces. Finally, the notation $\overline{c}$ denotes the lateral average of the concentration over the outer perivascular boundary. 
Both $\Omega$ and $\Lambda$ are here fixed in time. We remark that this model assumes that there is no interaction between the inner perivascular boundary and the blood vessel. 
...
\mer{MER: This we will probably have to condense. Move the detailed explanation to Supplemetary Methods?}
