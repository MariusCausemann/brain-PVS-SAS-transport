...
\begin{abstract}
  ...
  human brain health, with alterations often linked to
  ...
  we combine mixed-dimensional modelling, multi-modal magnetic 
  resonance images, and high performance computing to construct and 
  explore a high-fidelity in-silico model of human intracranial molecular enrichment. 
  This model predicts the temporo-spatial spreading of a solute
  within an image-derived geometric representation of the
  subarachnoid space, ventricular system and brain parenchyma,
  including networks of surface perivascular spaces
  ...
  in surface PVS networks that substantially enhances tracer
  enrichment, and that impaired enrichment is a direct
  ...
  exploring drug delivery and clearance in the human brain.
\end{abstract}
\begin{document}
...
parenchyma (\Cref{fig:results1}A--C, \Cref{fig:pvs_a}A) and includes
...
induces slow steady CSF flow of variable direction in these PVSs,
...
traveling waves of arterial wall motion (\Cref{fig:pvs_a}C), such as the pulse wave or other vasomotion~\cite{vanveluw2020vasomotion,
...
(\Cref{fig:pvs_a}D). The same theory predicts that strong ultraslow
...
reduced FTAs with higher PVS flow (\Cref{fig:pvs_a}B), up to 2.5 hours
...
(\Cref{fig:pvs_a}C, D), and especially so with higher PVS flow. These
...
47.0 (M1), 53.1 $\pm$ 50.5 (M2) and 82.6 $\pm$ 61.5 (M3) min after injection, thus with just a 15.3 min delay between M2 and M1. Comparing with our FTAs in the MCA segments, we note that the baseline model predicts a more-than 3 hour difference in FTA between the M1 and M2. Instead, the high PVS flow scenario predicts a 24 min delay between these segments.
Moreover, the high PVS flow scenario predicts FTAs of
84--108 min (M1) and 108--132 min (M2) after tracer appearance at the craniocervical junction, which overlaps with the previous ranges if one assumes a tracer travel time from the point of injection to the craniocervical junction of $13\,$min (as reported by Eide and Ringstad \cite{eide2024functional}). We interpret these results as indicative of a net CSF flow in human surface PVS beyond what would be induced by CSF production and cardiac peristalsis alone.
% M2 - M1 (mean) ~ 15.3 min 
...
$R_1^i$ (\Cref{fig:results1}C, \Cref{fig:pvs_a}A). We also assign an
...
$\Lambda_a$, linearly decreasing over the corresponding cell, to account for the transition from explicitly resolved
...
270053. 
M.C. acknowledges support from the RCN grant \#324239 (EMIx).
M.K. acknowledges support from the RCN grant \#303362.
The figures 1A, 1D, 2C, 3C, 5A and 6A were created in Biorender.
