...
First, we segment the structual T1 image using Synthseg \cite{billot2023robust,billot2023synthseg} to obtain masks of the parenchymal tissue and the CSF-filled spaces. In addition, we slightly modify the segmentation to ensure correct connectivity of the ventricular system. In particular, we elongate the aquaduct and enforce no communication between the ventricles and the surrounding CSF spaces except for the median aperture. The SAS is slightly dilated to ensure a physiological volume of the CSF space. After morphological smoothing of the segmented image, we extract surface representations of the outer SAS boundary, the pial membrane and the components of the ventricular system. From these surface representations, we create a tetrahedral mesh of both the parenchymal tissue and the CSF-filled spaces using the mesh generator fTetWild \cite{hu2020fast}. The resulting mesh consists of 97274 vertices and 536132 cells, which vary between $0.1$\,mm and $1.2$\,cm in cell diameter. The parenchyma has a total volume of $1382$\, ml, whereas the ventricles and the outer SAS contribute $32$\,ml and $277$\,ml to a total CSF volume of $309$\,ml, respectively.
\mar{this could also go into results. Keep it here for the moment?}

The computational domain $\Omega$ is defined by the union of the CSF spaces $\Omega_{\rm CSF}$ and brain parenchyma $\Omega_{\rm brain}$. The thin pia membrane encloses the brain, and thus the pial surface (of the brain) defines the CSF-brain interface $\Gamma_{\rm pia}$.
We mark the different regions of the boundary of $\Omega$, with
$\Gamma_{\mathrm{skull}}$ facing the dura and skull and $\Gamma_{\rm spine}$ representing the lower interface towards the spinal compartment (\Cref{fig:concept}).  The boundary of the spinal canal is denoted by $\Gamma
_{\rm{SC}}$ which is given by $\Gamma_{\rm {SC}} \backslash \Gamma_{{\rm brain}}$.  Further, we denote the surface of the lateral ventricles with $\Gamma_{\rm LV}$.
\mar{revisit once we have the concept figure.}
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.33\textwidth}
         \centering
         \includegraphics[width=\textwidth]{paper/figures/geometry_concept.png}
...
We model diffusion, convection and exchange of a solute in PVSs, in the surrounding SAS and in the brain parenchyma (Fig.~\ref{fig:concept}) via a comprehensive multi-dimensional transport model~\cite{masri2023modelling} over a timescale of minutes to hours. More specifically, we solve for a solute concentration $c$ (defined locally in each subdomain $\Omega_{\rm{brain}}$ and $\Omega_{\rm CSF}$ in the 3D SAS and brain~\cite{sykova2008diffusion} $\Omega$, and an averaged solute concentration $\hat c$ in a 1D network of centerlines $\Lambda$ representing the PVS, such that the following equations hold
\begin{subequations}
\begin{alignat}{2}
  \partial_t (\phi c) - \nabla \cdot (D \nabla (\phi c) ) + \nabla \cdot (\bm u c ) + \xi (\overline{c} - \hat c ) \delta_\Gamma & = f && \quad \quad \mathrm{in} \quad \Omega_{\rm CSF} \cup \Omega_{\rm{brain}}, \label{eq:multi_transport_3d} \\ 
...
Modelling an intrathecal injection, we prescribe a time-dependent solute influx rate on the interface towards the spinal compartment $\Gamma_{\mathrm{spine}}$
with the condition $D \nabla c \cdot \bm{n}  -  \bm u \cdot \bm n  c = g_{\mathrm{influx}}(t) \geq 0$. Here, we set
\begin{equation}
g_\mathrm{influx}(t) = \frac{m_{\rm tot}}{T_{\rm max}^2 |\Gamma_{\rm spine}|} \max(0, T_{\rm max} - |t - T_{\rm max}|)
\end{equation}
which results in a triangular inflow function with a peak at $T_{max} = 3600\,$s and a total tracer injection of $m_{tot} = 0.5$\,mM.

For the perivascular network, the boundary nodes are split into $\partial \Lambda^{\mathrm{in}}$  and $\partial \Lambda^{\mathrm{out}}$ based on the specified orientation of the network \mar{(figure?)}. We assume no tracer outflow on all boundary nodes and precribe a homogenous Neumann condition: $\hat D A \partial_s (\phi \hat c) - A \hat u \hat c  = 0 $ on $\partial \Lambda^{\mathrm{in}} \cup \partial \Lambda^{\mathrm{out}}$.

\mar{I assume that we need the split of the boundary nodes for the boundary conditions of the PVS flow models, oterwise we could furter simplify here...}
...
\begin{subequations}
    \begin{alignat}{2}
 - \mu \Delta \bm u_{\mathrm{sas}} + \nabla p_{\rm{sas}} & =  0 \quad && \mathrm{in} \,\,  \Omega_{\rm CSF}, \label{eq:momnetum_equation}  \\ 
 \nabla \cdot  \bm u_{\mathrm{sas}} & = g \quad && \mathrm{in} \,\,   \Omega_{\rm CSF}, \label{eq:divergence_equation}  \\ 
...
\end{equation}
 In the above $\bm u$ evaluates to $\bm u_{\rm{sas}}$ on $\Omega_{\rm CSF}$ and to zero otherwise, $\bm \tau_i$ is the tangent vector determined by the centerline $\Lambda_i$, $\Theta(s)$ is the cross-section of the PVS point at point $s$ and  $A$ is the area of this cross-section. \rami{to place a visualisation here too }
...
\begin{equation}
     \partial_t (\phi c) - \nabla \cdot (D \nabla (\phi c) ) + \nabla \cdot (\bm u c ) + \xi (\overline{c} - \hat c ) \delta_\Gamma = f \quad \quad \mathrm{in} \quad \Omega_{\rm CSF} \cup \Omega_{\rm{brain}}.  \label{eq:3d_pde}
\end{equation}
