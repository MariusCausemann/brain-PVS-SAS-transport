...
\mer{MER: WIP here. Next: need to update Figure references.}
...
supratentorial regions. Here, it spreads both posterior through the
quadrigeminal cistern and the longitudinal fissure, and anteriorly
through the outer SAS, reaching the top of the cerebral cortex after
around 12 hours. After 24 hours, the tracer covers most of the brain
surface (with the exception of some posterior regions) and has
penetrated substantially into the parenchymal tissue. This pattern is
reflected in the mean tracer concentrations in each compartment, where
the CSF space reaches its peak of $1.4\,$mmol/l after 2 hours,
followed by the arterial PVS concentration peak ($2.9\,$mmol/l after 4
hours) (\fixme{\Cref{fig:intrathecal}F}). While the mean
concentrations in these compartments drop soon after peaking, the
concentration in the parenchyma increases more slowly over the first
24 hours, with a final value of $0.3$ mmol/l. Overall, about 40\% of
the total amount of tracer remains in the cranium 24 hours
post-injection in this simulation, with the largest share in the CSF
(53\%), followed by the parenchyma (35\%), the venous PVS (8\%) and
the arterial PVS (5\%) (\fixme{\Cref{fig:intrathecal}G}).
...
\begin{figure}
    ...
    \includegraphics[width=0.7\textwidth]{figures/figure1.png}
     ...
     A) illustration of the intrathecal tracer injection protocol;  
     ...
     D) average tracer concentration in each compartment over the first 24\,h; 
     E) total amount of tracer in each compartment over the first 24\,h.}
...
To inform the convective velocity fields and dispersion factors
...
Traveling waves of arterial wall motion (\fixme{\Cref{fig:pvs}C}),
...
% Reduced CSF pulsatility strongly shifts intracranial tracer distribution patterns
\subsection*{Can variations in CSF convection and dispersion shift these tracer distribution patterns?} 

%% \begin{figure}%[h!]
%% \centering 
%% \includegraphics[width = 0.95\textwidth]{figures/figure2.png}
%% \caption{\textbf{The balance between CSF pulsatility and production shapes tracer distribution patterns.}
...
%% (\Cref{fig:csf}A--C).
%
%% Beginning with the advective, net flow field driven by CSF production,
%% we impose a constant CSF inflow of $400\,$ml/day across the surface of
%% the lateral ventricles. Simultaneously allowing for efflux across the
%% upper convexity yields a total CSF pressure drop of $26\,$mPa (0.00020
%% mmHg) (\Cref{fig:csf}D) with a maximum flow velocity of $1.85\,$mm/s
%% (in the aqueduct) and a mean velocity in the SAS of $2.6$\textmu m/s
%% (\Cref{fig:csf}E).
%
To address this question, we introduce three variations of the
baseline model: with (i) no CSF production, (ii) reduced
pulsatility/decreased dispersion, and (iii) higher
pulsatility/increased dispersion, respectively (\Cref{fig:csf}). We
...
respiratory pulsations (see Methods), which changed the dispersion
factors by 0.25$\times$ and 4$\times$, respectively. Note that this
behaviour directly reflects the nature of our CSF model model: Stokes
...
considerably delayed (\fixme{\Cref{fig:csf}}). Even after $12$ hours,
...
exclusively within the anterior regions (\fixme{\Cref{fig:csf}}). This
feature can be attributed to the CSF flow bifurcation posterior to the
ambient cistern (\fixme{\Cref{fig:results1}}): without sufficient
...
\caption{\textbf{Changes in the balance between convection and
    dispersion in the CSF spaces substantially alter computational
    predictions of tracer distribution patterns.} A) CSF flow induced
