...
in-silico predictions align well with the range of tracer distribution
...
Our key contribution is thus an in-silico framework that allows for
...
high accuracy using a mass-conserving finite element scheme. The
complete computational framework is implemented in the FEniCS finite
...
of auxiliary CSF and PVS flow models (\fixme{\Cref{fig:results1}},
\fixme{\Cref{fig:pvs}}). Beginning with flow field driven by CSF production,
...
$2.6$\textmu m/s (\fixme{\Cref{fig:results1}}). To estimate the dispersive
...
(\fixme{\Cref{fig:results1}}). The respiratory dispersive mixing reaches a
factor of up to 320 due the lower respiratory frequency of $0.25\,$Hz
(\fixme{\Cref{fig:results1}}). 
...
such a contrast-enhanced MRI protocol~\cite{ringstad2017glymphatic,
  ringstad2018brain, eide2024functional}(\fixme{\Cref{fig:intrathecal}C}), we
...
(\fixme{\Cref{fig:intrathecal}D--E}). To begin with, we consider a reasonably
...
whether CSF flows more rapidly in human PVSs stand as a point of
debate~\cite{bohr2022glymphatic, van2024caa, kipnis2025resolving}. In
light of the interest in this question,


here introduce a high PVS
flow model variation and compare the predicted tracer distribution
patterns between the baseline and the high PVS flow models.

here compare predictions
from the baseline model, accounting for cardiac peristalsis only, with
a high PVS flow model variation, adding also the estimated
contribution from ultraslow vasomotion (\fixme{\Cref{fig:pvs}X, Y}).
...
reduced FTAs with higher PVS flow (\Cref{fig:high_low_pvs}B), up to
...
especially so with higher PVS flow. These observations also hold on an
...
accelerated tracer transport with higher PVS flow, especially at
