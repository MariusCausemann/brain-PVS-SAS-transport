...
centerline of a blood vessel segment and labelled with its lumen
radius $R_1^i$. To represent annular PVSs, we also assign an outer PVS
radius $R_2^i > R_1^i$ for each segment $i$. The resulting
periarterial network graph consists of $12\,708$ edges connected at
$12\,562$ inner nodes and with $147$ end nodes, and the associated
domain is denoted by $\Lambda_a$. We identify three of the end nodes
-- corresponding to the two internal carotid arteries (ICAs) and the
basilar artery (BA) -- as root nodes, and designate the other ends as
leaf nodes. The resulting perivenous network graph consists of
