...
\subsection*{A computational framework for intracranial solute transport integrating surface perivascular networks}
...
(\Cref{fig:results1} and Methods). We introduce \fixme{three} solute
concentration fields, each varying in space and time, one in the 3D
domains and one in each of the PVS networks, and assume that the
solute can cross between the 3D and 1D domains via semi-permeable
...
account for two main mechanisms: (i) convection via a (small) net flow
...
CSF~\cite{sweetman2011cerebrospinal, vinje2019respiratory,
  ray2021quantitative} (\Cref{fig:results1}). Moreover, in the
periarterial spaces, we account for convection via directional (net)
fluid flow induced by peristaltic pumping~\cite{mestre2018flow,
...
time-averaged PVS flow to bridge temporal scales (see also
Limitations). Mathematically, the solute transport model is
represented by a mixed-dimensional system of coupled time-dependent
partial differential equations\cite{masri2024modelling}, which we
solve numerically with high accuracy using a mass-conserving finite
element scheme.

To inform the convective velocity fields and dispersion factors
entering as input parameters in the transport equations, we also
define a set of auxiliary CSF and PVS flow models
(\Cref{fig:results1}, \Cref{fig:pvs}). The periarterial network
extends from the internal carotid arteries and basilar artery through
up to 18 bifurcations to reach upstream network ends located within
the SAS or up to 6 mm inside the parenchyma and includes major surface
arteries of radius 0.5--1.4 mm (\Cref{fig:pvs}\fixme{?}). Due to
imaging limitations, the network primarily covers the supratentorial
regions, with the lowest network point located approximately $4\,$cm
above the craniocervical junction.

\mer{Just moved this from Results 3. Will edit, and maybe move some to
  Methods.}

\fixme{Applying a semi-analytic model of the net flow
  induced by peristalsis in perivascular
  networks~\cite{gjerde2023directional} (see Methods), we estimate
  that the cardiac pulse wave alone, traveling at a frequency of 1 Hz
  with a wavelength of 2.0 m and a 1\% wall
  ...
  (\Cref{fig:pvs}D,E). The same theory predicts that strong ultraslow
  vasomotions, if traveling antegrade at 0.1Hz with a wavelength of
  0.02 m and a 10\% displacement~\cite{broggini2024long}, will induce
  both retrograde and antegrade net PVS flow with an average velocity
  of 13.05 \textmu m/s and maximum velocity 54.44 \textmu m/s
  (\Cref{fig:pvs}D,F).}
...
cycles~\cite{sweetman2011cerebrospinal, vinje2019respiratory} and a
perivascular net flow induced by cardiac
pulsations~\cite{mestre2018flow, gjerde2023directional} on the order
of $\fixme{10.5}$ mL/day (see Methods).
...
\subsection*{Changes in the balance between CSF convection and dispersion substantially alter predicted tracer patterns}
...
differ substantially between subjects and in pathological
conditions~\cite{ringstad2018brain, eide2021direction,
  eide2021impaired, eide2022altered}. These neurological conditions
are also associated with alterations in the pulsatile flow of CSF in
the ventricular system and SAS~\cite{eide2021direction}. Moreover,
cardiac and respiratory pulsatility clearly varies from subject to
subject and between physiological states. Motivated by these
observations, we asked whether and how the tracer distribution
patterns predicted by our computational framework would be affected by
changes in CSF production or pulsatility.
%
...
directly reflects the nature of our model: Stokes flow is linear in
its inflow boundary conditions, and the diffusion enhancement factor
is quadratic in the predicted pressure gradient.
...
ambient cistern (\Cref{fig:csf}A): without sufficient diffusion, the
...
% MER: Moved from Results
\fixme{Beginning with the advective, net flow field driven by CSF
  production, we impose a constant CSF inflow of $400\,$ml/day across
  the surface of the lateral ventricles. Simultaneously allowing for
  efflux across the upper convexity yields a total CSF pressure drop
  of $26\,$mPa (0.00020 mmHg) (\Cref{fig:csf}D) with a maximum flow
  velocity of $1.85\,$mm/s (in the aqueduct) and a mean velocity in
  the SAS of $2.6$\textmu m/s (\Cref{fig:csf}E).} In model variations
with no CSF prodution, we set $u_{\rm CSF} = 0$, $p_{\rm CSF} = 0$.
...
% MER Moved from Results, needs editing.
\fixme{Next, to examine the
