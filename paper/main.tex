...
in the CSF spaces (see Methods, \Cref{fig:results1}B--C, \fixme{Figure SX}).
To model the convective flow field, we impose a constant CSF inflow of $400$ mL/day at the lateral ventricle surfaces, representing the average daily physiological CSF production rate \cite{lehtinen2013choroid}. This assumption yields a maximum flow velocity of $1.85\,$mm/s (in the aqueduct) and a mean velocity in the SAS of $2.6$ \textmu m/s
...
variations in diffusive permeability on molecular transport, separated
...
that molecular transport within the parenchyma is well-represented by
...
of molecular movement within the human brain environment.
...
network, and we thus set $\hat{u}_v = 0$ in $\Lambda_v$ in all simulations.
For the periarterial network, we account for cardiac peristalsis in the baseline model, and set $\hat{u}$ as $\hat{u}_a = \hat{u}_a^c$ in $\Lambda_a$. 
We remark that none of these models account for fluid exchange across the lateral outer interface of the PVSs (over $\Gamma_a, \Gamma_v$), while fluid entry and exit is
