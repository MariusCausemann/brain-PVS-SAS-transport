...
\subsection*{OLD: Perivascular flow shapes and accelerates molecular transport}
...
consider an integrative model of solute transport, a comprehensive
model of CSF and ISF flow is not within the scope of this work. In
particular, we here consider independent flow models in the CSF and
PVS, without coupling.

\mer{MER: Stopped here.}

\fixme{and moreover, evaluate their integrated effect in the context of
intracranial solute transport.}
...
\begin{figure}
    ...
    \caption{\fixme{\textbf{A static pressure gradient and peristaltic pumping are potential drivers of perivascular flow.}}
...
However, whether CSF flows more rapidly in human PVSs stands as an
open question~\cite{bohr2022glymphatic, van2024caa,
  kipnis2025resolving}. In light of this, we asked to what extent our
predicted tracer distributions would be affected by the flow in the
PVSs. Specifically, we here introduce a model variation with
substantially higher net flow in the PVS, by adding the contribution
from strong ultraslow vasomotion to that from the cardiac pulse wave
(\fixme{\Cref{fig:pvs}X, Y}), and compare with the baseline model.
