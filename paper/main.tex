...
(\fixme{\Cref{fig:results1}B--F}). Relative to this geometry, we define two
...
\begin{figure}%[h!]
\centering \includegraphics[width=0.8\textwidth]{figures/figure2.png}
...
to moderate (on average 0.92 \textmu m/s), but intriguingly, our
estimates of the contribution from vasomotion (13.05 \textmu m/s) are
...
\fixme{10.5--X} mL/day in the periarterial network without explicitly
...
such transport mechanisms within the parenchyma in future work. \fixme{We
also note that we observe tracer accumulation at the perivascular
network ends after 12 hours, similar to that previously observed for
microspheres~\cite{bedussi2018paravascular, mestre2018flow}, which
here can be interpreted as as an artifact of our modelling of the
solute exchange between the network ends and their surroundings,
creating a bottleneck for tracer movement.} To compensate, we have
focused on reporting detailed predictions in the CSF spaces and
surface PVS after not more than 24 hours.
...
concentration in the SAS with a rate constant $\beta_{\rm exit}$. \fixme{We
set a no-flux condition at the end nodes of the perivascular networks,
not allowing for direct solute efflux there.}
...
neither (see also Supplementary Methods). \fixme{To account for the
...
artificial tracer buildup. At the end nodes of $\Lambda_a$,
