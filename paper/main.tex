...
and modes of intracranial transport are under substantial
debate\cite{smith2019going, proulx2021cerebrospinal,
  bohr2022glymphatic, hladky2022glymphatic, betsholtz2024advances},
the target is to introduce a computational framework that allows for
...
subject-specific geometries. To this end, we introduce the following
modelling assumptions. We assume that the solute will diffuse within
all compartments, with diffusivity depending on the effective
...
transport, and we account for two main mechanisms: (i) convection via
a (small) net flow of CSF resulting from production in the choroid
plexus with CSF efflux across the upper
convexity\cite{hornkjol2022csf}, and (ii) dispersive effects resulting
from the pulsatile flow of CSF induced by the cardiac and respiratory
cycles\cite{sweetman2011cerebrospinal, vinje2019respiratory,
...
peristaltic pumping~\cite{mestre2018flow,
  gjerde2023directional}. Specifically, we use the
...
time-averaged PVS flow to bridge temporal scales; these choices are
...
scheme (see Methods). To inform the convective velocity fields and
dispersion factors entering as input parameters in the transport
equations, we also define a set of auxiliary CSF and PVS flow models
(see Methods). The computational framework is implemented in the
