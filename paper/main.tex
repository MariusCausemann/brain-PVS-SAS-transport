...
We consider a comprehensive time-dependent convection-diffusion model~\cite{masri2023modelling} of a solute concentration $c$ in a network representing the pial vasculature and surrounding perivascular spaces $\Lambda$ and in the surrounding three-dimensional SAS-brain domain $\Omega$ (Fig.~\ref{fig:concept}). More specifically, we solve for the concentrations $c$ (of the solute in the fluid space~\cite{sykova2008diffusion}) defined over the SAS-brain domain $\Omega$ and $\hat c$ in the perivascular network $\Lambda$ such that the following equations hold
\begin{subequations}
\begin{alignat}{2}
  \partial_t (\phi c) - \nabla \cdot (D \nabla (\phi c) ) + \xi (\overline{c} - \hat c ) \delta_\Gamma & = f && \quad \quad \mathrm{in} \quad \Omega, \\ 
  \partial_t (A \phi \hat c) - \partial_s(\hat D A \partial_s (\phi \hat c) + \xi P (\hat c - \overline{c})  &= A \hat f && \quad \quad \mathrm{in} \quad  \Lambda .
 \end{alignat}
\end{subequations}
Here $\phi$ is the fluid volume fraction or porosity of the brain (where $\phi \ll 1$), of the SAS (where $\phi = 1$) and of the perivascular space (where $0 \ll \phi \leq 1$), $D$ is the effective diffusion coefficient of the relevant solute in the respective media~\cite{sykova2008diffusion}, $\xi$ is a transfer coefficient across the outer perivascular boundary, $P$ and $A$ are the perimeter and area of the perivascular spaces, respectively, and $f$ and $\hat{f}$ represents given sources of solute in $\Omega$ and $\Lambda$, respectively. Both $\Omega$ and $\Lambda$ are here fixed in time.

\noindent \mer{MER: @Rami: I've added the $\phi$'s in this subsection. Please review and add convective terms in $\Omega$ and $\Lambda$.}
...
\begin{table}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Overview of physiological parameters and model variations. \mer{MER: This clearly needs some more work and probably would benefit from joint review. Should we look at it together, or does someone want to take the lead on collecting these?}}
  \label{tab:overview}
\end{table}
