...
(iii) be convected by a (small) net flow of CSF resulting from
...
(\Cref{fig:results1}E--G). Afer one hour, the in-silico
...
E) production-induced CSF velocities in the SAS and third and fourth ventricle (V3 \& V4), (frequency denotes the relative frequency of mean velocity magnitude values in each computational cell weighted by the cell volume); 
...
dispersive mixing reaches a factor of up to 320 due the lower
...
(\Cref{fig:pvs}B). On the other hand, traveling waves of arterial
...
vessels such as the MCA (\Cref{fig:pvs}D). The same theory predicts
...
maximum velocity $54.44$ $\mu$m/s (\Cref{fig:pvs}E).
...
artery after 48 minutes (\Cref{fig:pvs}F), defined by their
...
(\Cref{fig:pvs}J). Moreover, differences in tracer transport along
...
other arteries (\Cref{fig:pvs}L). In contrast, there are no clear
...
compared to the baseline (\Cref{fig:pvs}G+M), thus indicating a
...
\begin{figure}
    ...
    B) CSF production-driven flow field used to compute pressure-driven PVS flow (from \Cref{fig:csf}) and the relative freqency of PVS flow velocities (cellwise) in the network induced by a production-driven transmantle pressure gradient(positive values - antegrade, negative values - retrograde PVS flow);
...
\subsection*{Enlarged perivascular spaces yield substantially reduced periarterial transport}

Enlarged perivascular spaces are associated with cognitive decline and certain subtypes of dementia, and have been shown to impair periarteral tracer transport \cite{bown2022physiology,eide2024functional}. However, the exact mechanisms by which dilated PVS leads to reduced perivascular transport remains unclear. Here, we investigate the effect of perivascular enlargement on PVS flow velocities and the resulting changes in tracer enrichment patterns (\Cref{fig:enlargement}A).

By computing the pressure-driven, cardiac, and vasomotion-induced PVS flow fields in a perivascular geometry with an increased radius $R^{\rm enlarged}_{\rm PVS} = 3 R_{\rm vessel}$) compared to a healthy control ($R^{\rm control}_{\rm PVS} = 2 R_{\rm vessel}$), we observe notable changes in flow dynamics. Specifically, while the pressure-induced flow velocities increase with PVS dilation from a mean of $0.08\,$\textmu m to $0.33\,$\textmu m, the cardiac and vasomotion-induced mean velocities drop considerably, from $0.92\,$\textmu m to $0.17\,$\textmu m and from $13.05\,$\textmu m to $2.33\,$\textmu m, respectively (\Cref{fig:enlargement}B,C,D). These changes in flow velocities result in substantial alterations in tracer enrichment within the dilated PVS. Notably, the reduction in peristaltic PVS flow velocities leads to significantly delayed first-time arrivals (FTAs) in the presence of vasomotion and, to a lesser extent, in the baseline model with pressure-driven and cardiac-induced PVS (\Cref{fig:enlargement}E,F). Furthermore, the larger volume of the dilated PVS results in a greater accumulation of total tracer in the PVS, particularly in the vasomotion-induced high PVS flow velocity model, while the mean concentration slightly drops (\Cref{fig:enlargement}G,H).

\begin{figure}
    \centering
    \includegraphics[width=1 \textwidth]{figures/figure5.png}
    \caption{\textbf{Perivascular enlargement reduces PVS flow and tracer transport}
    A) illustration of dilated PVS; 
    B) mean and max pressure-driven and cardiac- and vasomotion induced flow velocities in healthy control ($R^{\rm control}_{\rm PVS} = 2 R_{\rm vessel}$) and  dilated PVS ($R^{\rm enlarged}_{\rm PVS} = 3 R_{\rm vessel}$); 
    C)+D) 3D rendering of vasomotion-induced PVS flow in a undilated and dilated PVS network; 
    E) Zoom-in to the region surrounding the MCA-R after 1, 2, 3 and 4\,h for the high PVS flow velocity model with and without dilated PVS. Innermost circle - artery (blue), surrounding annulus - PVS (color coded by concentration value), surrounding domain - parenchyma and CSF(color coded by concentration), separated by pia (blue: towards the parenchyma, pink: towards the CSF); 
    F) First-time arrival (time at which concentration exceeds $0.1\,$mmol/l for the first time) for different branches of the arterial tree for the conservative baseline model (cardiac and pressure-driven PVS flow) and the high PVS flow model (pressure-driven, cardiac and vasomotion PVS flow) for undilated and dilated PVS;
    G) total content of tracer in CSF, parenchyma and arterial and venous PVS at 1,3,6,12 and 24\,h for the four models from F);
    H) mean concentrations in CSF, parenchyma and arterial and venous PVS for the four models from F);
    }
...
We presented a holistic and integrated in-silico model of molecular enrichment in human intracranial spaces, enabling individualized predictions of molecular enrichment influenced by physiological factors such as respiration and cardiac pulsatility, as well as pathological conditions like enlarged perivascular spaces.
A key observation from this model is that the balance between CSF production and intracranial pulsatility significantly shapes the large-scale features of molecular enrichment patterns, explaining the substantial individual variations observed clinically \cite{ringstad2018brain}.
Even slow mean CSF flow rates (0.1--1.0 \,\textmu m/s) in the surface PVS result in earlier tracer enrichment around major cerebral arteries, with the effect being more pronounced—up to a three-hour difference in time of arrival—at higher PVS flow rates of approximately 10 \,\textmu m/s. 
Our rigorously derived models of PVS flow in perivascular networks, based on first principles, naturally yield both retrograde and antegrade flow, occurring with or without pressure differences at the network ends. 
The PVS may function as highways facilitating rapid transport, even in the absence of structural barriers, though a sharp concentration difference between the PVS and surrounding tissues is unlikely without such barriers.
Additionally, dilated PVS leads to a substantial reduction in cardiac- and vasomotion-driven flow velocities, which impedes perivascular transport. 
%However, the enlargement of the PVS results in a greater accumulation of perivascular tracer. 
...
More references on pressure difference values across the aqueduct:
