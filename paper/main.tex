...
\begin{itemize}
    \item Perivascular enlargement is associated with aging and certain subtypes of dementia and iNPH, and impairs periarteral tracer transport \cite{eide2024functional}. However, it is unclear how the dilated PVS leads to a reduction in perivascular transport. Here, we investigate the effect of perivascular enlargement on PVS flow velocities and the resulting changes in tracer enrichment patterns.
    \item We compute the pressure-driven, cardiac and vasomotion PVS flow fields in a perivascular geometry with increased radius ($R^{\rm enlarged}_{\rm PVS} = 3 R_{\rm vessel}$ vs $R^{\rm control}_{\rm PVS} = 2 R_{\rm vessel}$).
    \item while the pressure induced flow velicities increased with PVS dilation from .. to ...,  the cardiac and vasomotion induced velocities dropped considerably ....
    \item We find substantial changes in tracer enrichment with PVS dilation (\Cref{fig:enlargement} Zoom-in).
    \item The reduction in peristaltic PVS flow velocities leads to substantially longer FTAs in the presence in vasomotion, while the basline model (with pressure-driven and cardiac-induced PVS) is insensitive to the perivascular enlargement. (\Cref{fig:enlargement}).
    \item The larger volume of the PVS leads to an increase of perivascular tracer content, while the concentration is slightly lower.
\end{itemize}
