...
The cardiac and respiratory cycles (typically 0.25--1.0 Hz in humans)
induce pulsatile flow of CSF in the ventricular system and in the
cranial and spinal SAS. Pulsatile flow leads to dispersion which in
turn may enhance molecular transport~\cite{taylor1953dispersion,
...
dispersive effects over a longer time scale (hours to days) and with
spatial differences, we propose the following four-step approach for
the cardiac and respiratory contributions separately.
\begin{enumerate}%[\label=\roman]
\item
  To account for viscous forces, we compute spatially-varying CSF
  pressure fields $p^0_c, p^0_r$ in $\Omega_{\rm CSF}$ corresponding to the
  Stokes flow induced by the peak volumetric reduction of the CSF
  space in the respective cycle. This step is described in further
  detail below.
\item
  Next, we estimate the Womersley numbers $\alpha_c, \alpha_r$
  associated with the cardiac and respiratory flow patterns by the
  definition
  \begin{equation}
...
\item
  ...
  use the respective Womersley number $\alpha_i$ to calculate an
  upscaled pressure field $p^1$ from $p^0$:
  \begin{equation}
    p^1_i(x) = \left ( 1 + \frac{\alpha_i^2}{8} \right ) p_i^0(x)
    \qquad \text{for } x \in \Omega_{\rm CSF} .
  \end{equation}
\item
  Further, assuming unsteady dispersion, we follow Sharp et
  al.~\cite{keith2019dispersion} to estimate local enhancement factors
  $S_i$ from the non-dimensionalized pressure gradients:
  \begin{equation}
    S_i = \frac{1}{\alpha_i^3} \frac{1}{\omega_i \mu/h} | \nabla p_i^1 |.
  \end{equation}
\item
  Finally, we account for the non-local nature of dispersion by
  smoothing ...
\end{enumerate}
