...
\end{equation}
Since $\bm V_h \subset H(\mathrm{div}, \Omega_{\rm CSF})$, then $[\bm v_n] = 0 $ on $\mathcal{F}_{i, \rm CSF}$. Continuity in the tangential component is enforced weakly via interior penalization. Facets lying on the pial interface $\Gamma_{\rm pia}$ are
...
$\mathcal{F}_{\rm SSAS}$.
\discuss{For convenience, we denote
...
\begin{align}
  ...
  + \sum_{F \in \discuss{\mathcal{F}_{\rm AM-U}}} \int_{F} R_0 (\bm u_h \cdot \bm n) (\bm v_h \cdot \bm n)
