...
%% For a given edge $\Lambda^i= (\mathsf{v}_\mathrm{in}^i, \mathsf{v}_{\mathrm{out}}^{i})$, we define the function $n^i: \mathcal{V} \rightarrow \{-1,0,1\}$ with $$
%% $$
...
%% \begin{equation}
%% \sum_{ i \in \mathcal{E}(\mathsf{v})} (\hat D A_i \phi \partial_s \hat{u}_i(\mathsf{v})  -  A_i \hat u_i(\mathsf{v}) \hat c_i(\mathsf{v})) n^i(\mathsf{v}) = 0  \label{eq:flux_conservation}
%% \end{equation}
...
$\mathcal{F}_{\rm AM}$, on the interface toward the spinal cord
$\Gamma_{\rm SC}$ by $\mathcal{F}_{\rm SC}$ and on the interface
towards the spinal SAS $\Gamma_{\rm SSAS}$ by $\mathcal{F}_{\rm
  SSAS}$. The union of all facets in $\mathcal{T}$ is denoted by
$\mathcal{F}$.
...
space to capture the sharp boundary layers.
...
pointing from $E^1$ to $E^2$, the jump $[\cdot]$ is given by $[v] =
v|_{E^1} - v|_{E^2}$, and the weighed average $\{ \cdot \}_w$ is
defined as:
\begin{equation}
  \{v\}_{w} = \frac{\kappa_2}{\kappa_1 + \kappa_2} v |_{E^1} + \frac{\kappa_1}{\kappa_1 + \kappa_2} v |_{E^2},
  \quad \text{where} \quad \kappa_i = \phi D|_{E^i}.  
\end{equation}
The parameter $\gamma_{D,F}$ is the harmonic mean of the \discuss{porosity-weighted} diffusion coefficient given by 
\begin{equation}
  \gamma_{D,F} = \frac{\kappa_1\kappa_2}{\kappa_1 + \kappa_2},
\end{equation}
and $\eta$ is a user-specified penalty parameter.

\mer{MER. Stopped here.}

For the convection term, we use upwinding, see \cite[Section
  2.3.1]{di2011mathematical} and the references therein. For discrete
functions $c, v \in V_h^k$, define
\begin{equation*}
