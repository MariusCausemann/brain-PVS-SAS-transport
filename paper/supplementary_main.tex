...
$\mathcal{T}$, write $\hat{c}_i = \hat{c}|_{\Lambda^i}$, and let
...
\begin{equation}
  \hat{c}_i (y) = \hat{c}_y \qquad \foralls \Lambda^i \in \mathcal{E}(y),
...
each $y \in \mathcal{B}$:
\begin{equation}
  \sum_{i \in \mathcal{E}(y)} (\hat{D} A_i \partial_s \hat{c}_i (y)  -  A_i \hat u_i(\mathsf{v}) \hat c_i(\mathsf{v})) n_i(\mathsf{v}) = 0
...
On the interface $\Gamma_{\mathrm{pia}}$ between the CSF and brain, we prescribe the following interface condition modeling a semi--permeable interface, writing $c|_{\Omega_{\rm PAR}} = c_{\rm PAR}$ and $c|_{\Omega_{\rm CSF}} = c_{\rm CSF}$:
\begin{subequations}
\begin{alignat}{2}
...
\end{alignat} 
\end{subequations}
Here, $\beta_{\mathrm{pia}}$ is a permeability constant between the
brain and the CSF. \mer{MER: Is the inclusion of $\Gamma_{\rm LV}$
  correct in the equation here? If so, text needs minor edits.}

For the perivascular network, the boundary nodes are collected into
$\partial \Lambda$. We assume no tracer outflow on all boundary nodes
and prescribe a homogeneous Neumann condition:
\begin{equation}
