...
\begin{equation}
  \hat{c}^i (y) = \hat{c}_y \qquad \foralls \Lambda^i \in \mathcal{E}(y),
  \quad \foralls y \in \mathcal{B} .
\end{equation}
Moreover, to ensure conservation of flux, we set that the flux going
in and out at nodes should add to zero, or more precisely, that for
each $y \in \mathcal{B}$:
\begin{equation}
  \sum_{i \in \mathcal{E}(y)}
...
To model flow induced by pressure differences between end nodes in a
perivascular network $\Lambda$, with segments $\Lambda^i$, internal
nodes $\mathcal{B}$ and end nodes $\mathcal{T}$, we consider the
following system of hydraulic network
equations~\cite{daversin2022geometrically, gjerde2024directional}. The
unknowns are the PVS velocity $u_{\rm pvs}$ and pressure $p_{\rm
  pvs}$, which represent the fluid velocity in the axial direction
averaged across the PVS cross-sections and the cross-section-average
fluid pressure, respectively, solving
