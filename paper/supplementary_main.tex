...
$\mathcal{F}_i = \mathcal{F}_{i, \rm PAR} \cup \mathcal{F}_{i, \rm
  CSF}$. Facets lying on the pial interface $\Gamma_{\rm pia}$ are
denoted by $\mathcal{F}_{\rm pia}$, on the outer (arachnoid) boundary
$\Gamma_{\rm AM}$ by $\mathcal{F}_{\rm AM}$, on the inner ventricular
interface $\Gamma_{\rm LV}$ by $\mathcal{F}_{\rm LV}$, on the boundary
toward the spinal cord $\Gamma_{\rm SC}$ by $\mathcal{F}_{\rm SC}$ and
on the boundary towards the spinal SAS $\Gamma_{\rm SSAS}$ by
$\mathcal{F}_{\rm SSAS}$. \discuss{For convenience, we denote
  $\mathcal{F}_e = \mathcal{F}_{\rm pia} \cup \mathcal{F}_{\rm LV}
  \cup \mathcal{F}_{\rm AM-U} \cup \mathcal{F}_{\rm SSAS}$, and the
  union of all facets in $\mathcal{T}$ is denoted by $\mathcal{F} =
  \mathcal{F}_i \cup \mathcal{F}_e \discuss{\cup \mathcal{F}_{\rm SC}
    \cup \mathcal{F}_{\rm AM-L}}$.}
...
\begin{equation}
  ...
  V_h^k = \{c \in L^2(\Omega): \;  c \vert_E \in \mathbb{P}^k(E), \; E \in \mathcal{T}\} ,
\end{equation}
where $L^2(\Omega)$ is the space of square-integrable functions
defined over $\Omega$ and $\mathbb{P}^k(E)$ denotes the space of
...
pointing from $E^1$ to $E^2$, \discuss{$h_F$ denotes the facet diameter},
...
\end{equation}
Since $\bm V_h \subset H(\mathrm{div}, \Omega_{\rm CSF})$, then $[\bm v_n] = 0 $ on $\mathcal{F}_{i, \rm CSF}$. Continuity in the tangential component is enforced weakly via interior penalization. Define the form \mer{MER: Check $\Gamma_{\rm AM}$ part. 26.11.24: Rami will check $\mathcal{F}_e$ versus definitions of the boundaries and where it appears.}
\begin{multline}
  \mathcal{A}_h(\bm u, \bm v)
  = \sum_{E \in \mathcal{T}_{\rm CSF}} \int_{E} \mu \nabla \bm u : \nabla \bm v
  - \sum_{F \in \discuss{\mathcal{F}_{i, \rm CSF} \cup \mathcal{F}_{e}}} \left (
...
CSF pulsates in the SAS and ventricular system with each cardiac and respiratory cycle. Taking the large spatial differences of pulsatile CSF flow into account, we estimate a local diffusion enhancement factor $R$ in the CSF-filled space $\Omega_{\rm CSF}$ by combining a computational model of cardiac- and respiration-induced pulsatile CSF flow with theoretical estimates for shear-augmented (Taylor) dispersion \cite{taylor1953dispersion, watson1983diffusion}.

\rami{I rewrote the above paragraph and some aspects of this section}

\rami{  CSF pulsates in the SAS and ventricular system with each cardiac and respiratory cycle with large spatial differences.  This leads to dispersion effects. For example, 
...
This phenomena is accounted for in our model by scaling the free diffusion coeffiecient $D^{\mathrm{Gad}}$ by the formula $D^{\mathrm{eff}} = (1+R) D^{\mathrm{Gad}}$ where $R$ represents a local diffusion enhancement in the CSF-filled space $\Omega_{\rm CSF}$.} 

\rami{ Note that since there is no data for the local values of $R$, we estimate $R$ by combining computational results (solving Stokes flow equations, see below) and explicit estimates for dispersion enhancement that are derived under simplifying assumptions on the problem geometry, the flow field and concentration profiles \cite{watson1983diffusion, keith2019dispersion}. Therefore such an estimate is heuristic; however,  our computational results qualitatively agree with observed data.  A key feature in our estimate of $R$ is that it is spatially varying, including local effects depending on a  computed flow field. }


Similar to the case of CSF flow driven by production introduced in \Cref{sec:csf_flow}, we employ a Stokes flow model to predict CSF velocities corresponding to the peak cardiac and respiratory blood inflow rates. In particular, we enforce a uniformly distributed inflow of $u^{\rm in}_{\rm AM}$ and $u^{\rm in}_{\rm LV}$ across the arachnoid mater ($\Gamma_{\rm AM}$) and lateral ventricular surfaces ($\Gamma_{\rm LV}$) respectively, mimicking the effect of reduced CSF volume due to blood inflow. We set $u^{\rm in}_{\rm AM} = 6\,$ml/s and $u^{\rm in}_{\rm LV}=0.31\,$ml/s in the cardiac case, and $u^{\rm{in}}_{\rm{AM}} = 1\,$ml/s and $u^{\rm in}_{\rm{LV}}=0.121\,$ml/s in the respiratory case, respectively.
...
\begin{equation}\label{eq:cardiac_flow_bcs}
    ...
    \bm u = \frac{u^{\rm in}_{\rm AM} \cdot \bm n}{|\Gamma_{\rm skull}|} \text{ on } \Gamma_{\rm skull}.
\end{equation}
The obtained pressure fields $p_{\rm cardiac}$ and $p_{\rm resp}$ only account for viscous forces, albeit cardiac and respiratory induced CSF flow is known to be unsteady and substantially impacted by transient inertial forces. Assuming an oscillatory flow with either the heartbeat frequency $\omega_{\rm cardiac} = 2 \pi$ or respiratory frequency $\omega_{\rm resp} = 0.5 \pi$, a mean gap width of the CSF-filled spaces of $2h=3$\,mm and CSF density $\rho=10^3\,\text{kg/m}^3$, the Womersley numbers $\alpha$ become $\alpha_{\rm cardiac}^2 = \frac{h^2 \omega_{\rm cardiac} \rho}{\mu} = 20.2$ and $\alpha_{\rm resp}^2=5.05$, which is unsteady. Based on theoretical considerations on the ratio of oscillatory flow to steady flow impedances in a tube, we account for inertial forces in the pulsatile flow by upscaling the computed viscous pressure field by a factor of $1 + \alpha^2 / 8$ (see \cite{van1998cardiovascular}, chapter. 4.3). Thus, the upscaled pressure fields become $p^{\rm up}_{\rm cardiac}= (1 + \alpha_{\rm cardiac}^2 / 8)p$ and $p^{\rm up}_{\rm resp}= (1 + \alpha_{\rm resp}^2 / 8)p$.

Further assuming unsteady dispersion, we follow Sharp et al \cite{keith2019dispersion} and estimate each enhancement factor $R_{\rm cardiac}$ and $R_{\rm respiratory}$ from the non-dimensionalized pressure gradient 
