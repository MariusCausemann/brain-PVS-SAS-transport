...
See~\Cref{tab:scenarios} for an overview of the different in-silico
scenarios and computational models considered.
\begin{table}
  \begin{center}
  \begin{tabular}{ll|llllll|ll}
    ...
    \midrule
    A & Baseline & $R_2 = 2 R_1$ & $\xi$ & $10^{-4}$ mm$^2$/s\cite{hornkjol2022csf} & $\phi R D^{\rm gad}_{\rm eff}$\cite{sykova2008diffusion, valnes2020apparent} & $\hat{u}_{\rm prod} + \hat{u}_{\rm cardiac}$ & $\mathbf{u}_{\rm prod}$ & 0 \\ 
...
average of the path radii. \discuss{For the few (2) nodes in $\Lambda$ with
