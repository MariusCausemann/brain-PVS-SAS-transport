...
The morphological parameters $\lambda$ and $N$ are chosen based on established quantitative measurements of the cerebral microvasculature \cite{cassot2006novel, lorthois2014tortuosity}. For a typical terminal node with radius $R_1 = 0.5\,$ mm, we set the length-to-radius ratio to $\lambda = 16$ \cite{lorthois2014tortuosity}. Resolving the network down to $N = 15$ unresolved branching levels scales the terminal vessels to approximately $15 \, \mu$m, matching the characteristic dimension of a precapillary arteriole.
