...
Finally, to augment~\eqref{eq:1d_pde} at the network end nodes $y_k \in \mathcal{N}$, we represent the unresolved, continuous extension of the perivascular networks by coupling these tips to a set of 0D compartments. For each terminal node $y_k \in \mathcal{N}$, let $c^0_k(t)$ denote the solute concentration in a 0D compartment of effective volume $V_k$ (maybe just $|B_k|$, or weighted with fluid volume fraction $\phi$?). This compartment exchanges mass with the 3D domain over a local spherical neighborhood $B_k$ (with volume $|B_k|$) centered at $y_k$.
...
\begin{equation}
  S_{\text{0D} \to \text{3D}}(x, t) = \sum_{y_k \in \mathcal{N}} \frac{\beta_k}{|B_k|} (c^0_k - c) \chi_{B_k}(x) .
  \label{eq:3d_source}
\end{equation}
