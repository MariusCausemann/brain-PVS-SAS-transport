...
Finally, to augment~\eqref{eq:1d_pde} at the network end nodes $y_k \in \mathcal{N}$, we represent the unresolved, continuous extension of the perivascular networks by coupling these tips to a set of 0D compartments, similar to the approach in \cite{fritz20221d}. For each end node $y_k \in \mathcal{N}$, let $c^0_k(t)$ denote the solute concentration in a 0D compartment of effective volume $V_k$. This compartment exchanges mass with the 3D domain over a local spherical neighborhood $B_k$ (with volume $|B_k|$) centered at $y_k$. 

The rate of change of mass in the 0D compartment is governed by advective and diffusive fluxes from the 1D network, alongside diffusive exchange with the 3D tissue:
\begin{equation}
  V_k \frac{d c^0_k}{dt} = q_k^+ \hat{c}(y_k) + q_k^- c^0_k + \alpha_k (\hat{c}(y_k) - c^0_k) + \beta_k ( \bar{c}_{B_k} - c^0_k ) \quad \forall y_k \in \mathcal{N},
  \label{eq:0d_ode}
\end{equation}
where $q_k = (A\hat{u})(y_k) \cdot n(y_k)$ is the volumetric fluid flow rate leaving the 1D network at $y_k$ (with $n(y_k)$ denoting the outward normal), split into outflow $q_k^+ = \max(q_k, 0)$ and inflow $q_k^- = \min(q_k, 0)$ components to implement upwinding, and $\alpha_k$ is the 1D-0D diffusive conductance. $\beta_k$ is the 0D-3D diffusive exchange rate, and $\bar{c}_{B_k}(t) = \frac{1}{|B_k|} \int_{B_k} c(x, t) \, dx$ is the average 3D concentration within $B_k$.
...
\begin{equation}
  -\hat{D}A \partial_{n} \hat{c} = (q_k^- - \alpha_k)( c^0_k - \hat{c} ) \quad \mathrm{on} \quad y_k \in \mathcal{N}.
  \label{eq:1d_bcs}
\end{equation}
...
Finally, the diffusive exchange between the 0D compartments and the 3D domain acts via a localized volumetric source term $S_{\text{0D} \to \text{3D}}$ in~\eqref{eq:3d_pde}, defined using the indicator function $\chi_{B_k}(x)$ for each ball $B_k$:
\begin{equation}
  S_{\text{0D} \to \text{3D}}(x, t) = \sum_{y_k \in \mathcal{N}} \frac{\beta_k}{|B_k|} (c^0_k - \bar{c}_{B_k}) \chi_{B_k}(x) .
  \label{eq:3d_source}
\end{equation}
