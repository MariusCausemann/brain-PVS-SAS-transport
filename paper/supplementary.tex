...
\title{Supplementary information for In-silico molecular transport via perivascular networks in the human intracranial space}
...
\begin{abstract}
  ...
  \emph{In-silico molecular transport via perivascular networks in the human intracranial space}.
\end{abstract}
...
\subsection{Nomenclature and notation} We begin by including \Cref{table:geometry_notation}, \Cref{table:variable}, and \Cref{table:mesh}, which summarize the common notation used for the geometry, model variables and parameters, and finite element discretization, respectively.
\begin{table}[h!] 
\centering
...
$L_i$ & Length of a PVS segment $\Lambda_i$ \\
...
% $\widehat V_{h,0}$, $\widehat V_{h,g}$ & Continuous Galerkin space on network with homogeneous or prescribed BCs \\
...
$\widehat{Z}_{h}(\mathcal{I})$ & Space of piecewise constants ($P^0$) on network mesh $I$ \\
$\tau$ & Time step size \\
\hline
\end{tabular}
\end{table}
