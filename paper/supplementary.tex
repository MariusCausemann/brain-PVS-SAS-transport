...
\begin{equation}
  -\hat{D}A \partial_{n} \hat{c} = (q_k^- - \alpha_k)( c^0_k - \hat{c} ) \quad \mathrm{on} \quad y_k \in \mathcal{N}_{\rm leaf}.
  \label{eq:1d_bcs}
\end{equation}
We note that if advection is purely outward ($q_k > 0$, so $q_k^- = 0$), the boundary condition simplifies to $-\hat{D}A\partial_{n}\hat{c} = \alpha_k(\hat{c} - c^0_k)$, indicating that the diffusive flux at the boundary is driven purely by the concentration difference between the two compartments. Conversely, if $q_k < 0$ (flow enters 1D from 0D), the boundary condition balances the total diffusive flux with both the advective influx from the 0D compartment and the 1D-0D diffusive exchange. 
...
\begin{equation}
  \hat{D}A \partial_{s} \hat{c} - (A \hat{u} n(y_k)) \hat{c} = 0 \quad \mathrm{for} \quad y_k \in \mathcal{N} \setminus \mathcal{N}_{\rm leaf}.
  \label{eq:1d_noflux}
\end{equation}
