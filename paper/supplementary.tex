...
\title{\textcolor{black}{Supplementary information for In-silico solute transport via perivascular networks in the human intracranial space}}
...
\begin{abstract}
  Supplementary methods and discussion accompanying the main text of
  \textcolor{black}{In-silico solute transport via perivascular networks in the human intracranial space}.
\end{abstract}
...
\Cref{sec:app:peristalsis} are beyond the temporal resolution of our
...
notation, simply write $s \in \Lambda$ to represent the point ${\boldsymbol{\uplambda}}(s)$ on $\Lambda$ corresponding to $s$. Finally, we view
...
$\mathcal N\subset  \mathcal V$ & End nodes \\
\bottomrule
\end{tabular}
...
%% $\mathcal{T}_{\mathrm{PAR}},\,\mathcal{T}_{\mathrm{CSF}}$ & Restrictions of $\mathcal T$ to $\Omega_{\mathrm{PAR}},\Omega_{\mathrm{CSF}}$ \\
...
%% $V_{h}(\mathcal{T})$ & Space of piecewise linear polynomials on $\mathcal{T}$\\
%% $Q_{h}(\mathcal{T})$ & Pressure space on $\mathcal T_{\mathrm{CSF}}$  \\
%% $\mathcal{I}_a, \mathcal{I}_v$ & 1D meshes of arterial/venous networks $\Lambda_{a},\Lambda_{v}$ \\
%% $\widehat V_{h}(\mathcal{I})$ & Space of continuous piecewise linear polynomials on $\mathcal{I}$, $\mathcal{I}$ omitted when superfluous \\
%% $\widehat{Z}_{h}(\mathcal{I})$ & Space of piecewise constants ($P^0$) on $\mathcal{I}$, , $\mathcal{I}$ omitted when superfluous \\% $\widehat V_{h,0}$, $\widehat V_{h,g}$ & Continuous Galerkin space on network with homogeneous or prescribed BCs
%% $\mathrm{BDM}_{2}$ & Brezzi–Douglas–Marini space of degree 2 \\
%% $\tau$ & Time step size \\
%% \hline
%% \end{tabular}
%% \end{table}
...
% \subsection{Computational mesh entities: definitions and common notation}
...
\subsubsection{The incompressible Stokes equations}
...
\begin{subequations}
  \begin{alignat}{2}
    ( \mu \nabla \mathbf{u} \cdot \mathbf{n} - p \mathbf{n}) \cdot \mathbf{n}  &= -R_{\rm CSF, 0} ( \mathbf{u} \cdot \mathbf{n} ), 
    \quad \mathbf{u} \cdot \mathbf{t} = 0
    && \text{ on } \Gamma_{\mathrm{AM-U}}, \\
    \mathbf{u} &= 0 
    && \text{ on } \Gamma_{\mathrm{AM-L}}\cup\Gamma_{\rm{pia}}\cup\Gamma_{\rm{SSAS}}, \\
    %\mathbf{u} &= 0 && \text{ on } \Gamma_{\rm{pia}}, \\
    %\mathbf{u} &= 0 && \text{ on } \Gamma_{\rm{SSAS}}, \\
    \mathbf{u} \cdot \mathbf{n} & = \frac{1}{ |\Gamma_{\rm LV}|}  u_{\rm in}, 
    \quad \quad \mathbf{u} \cdot \mathbf{t} = 0 \quad 
    && \text{ on } \Gamma_{\rm{LV}} ,  
  \end{alignat}
  \label{eq:stokes_bcs}%
\end{subequations}%
where $\mathbf{n}$ and $\mathbf{t}$ denote the unit outward normal and tangent vectors to the boundary respectively,  $\mu$ is the (dynamic) CSF viscosity, $R_{\rm CSF, 0} > 0$ is a resistance parameter for CSF efflux, and $u_{\rm in}$ is a given fluid influx, here across the lateral ventricle wall $\Gamma_{\rm LV}$. We consider alternative variations of these boundary conditions in connection with estimating dispersion effects induced by CSF pulsatility, see~\Cref{sec:app:dispersion}.

\subsubsection{Finite element solution of the incompressible Stokes equations}
We consider an H(div)-based finite element approximation of the incompressible Stokes equations~\eqref{eq:stokes} defined over $\Omega_{\rm CSF}$ with the boundary conditions~\eqref{eq:stokes_bcs}. Following\cite{hong2016robust}, we approximate the velocity field $\mathbf{u}$ and the pressure field $p$ with the following finite element spaces:
\begin{align*}
  \mathbf{V}_{h,g} &= \{ \mathbf{v}  \in H(\mathrm{div}, \Omega_{\mathrm{CSF}}): \;
  \mathbf{v} \vert_{E} \in \mathrm{BDM}^2(E), \; E \in \mathcal{T}_{\rm CSF}; \;
  \mathbf{v} \cdot \mathbf{n} = 0
  \text{ on } \Gamma_{\rm{pia}} \cup \Gamma_{\rm AM-L} \cup \Gamma_{\rm{SSAS}}, \;\; \mathbf{v} \cdot \mathbf{n} =  g %\frac{1}{|\Gamma_{\mathrm{LV}}|} u_{\mathrm{in}} 
  \text{ on } \Gamma_{\mathrm{LV}} \}  \\ 
  Q_h  &= \{q \in L^2(\Omega_{\rm CSF}): \; q \vert_E \in \mathrm{P}^1(E),
  \; E \in \mathcal{T}_{\rm CSF} \}. 
\end{align*}
Here, $H(\mathrm{div}, \Omega_{\rm{CSF}})$ is the space of $L^2(\Omega_{\rm{CSF}})$ vector fields with $L^2(\Omega_{\rm{CSF}})$ divergence, $\mathrm{BDM}^2$ is the Brezzi-Douglas--Marini element~\cite{brezzi1987mixed} of degree 2, $g$ is a given constant, and $\mathbf{n}$ is the unit outward normal vector to each facet. Given any vector $\mathbf{v}$, the normal and tangential components on each facet are denoted and given by 
$$ \mathbf{v}_n = (\mathbf{v} \cdot \mathbf{n}) \mathbf{n}, \quad \mathbf{v}_t = \mathbf{v} - \mathbf{v}_n. 
$$ 
Since $\mathbf{V}_{h,g} \subset H(\mathrm{div}, \Omega_{\rm CSF})$, then $[\mathbf{v}_n] = 0 $ on $\mathcal{F}_{i, \rm CSF}$, the interior facets to $\Omega_{\mathrm{CSF}}$. Continuity in the
...
\begin{multline}
  \mathcal{A}_h(\mathbf{u}, \mathbf{v})
  = \sum_{E \in \mathcal{T}_{\rm CSF}} \int_{E} \mu \nabla \mathbf{u} : \nabla \mathbf{v}
 + \sum_{F \in \mathcal{F}_{i, \rm CSF} \cup \mathcal{F}_{e}} \left (
  -\int_{F} \mu \{\nabla \mathbf{u}\}  \mathbf{n}_F \cdot [\mathbf{v}_t] 
  - %\sum_{F \in \mathcal{F}_{h,c} \cup \mathcal{F}_{\Gamma}}
  \int_{F} \mu \{\nabla \mathbf{v}\}  \mathbf{n}_F \cdot [\mathbf{u}_t]
  + %\sum_{F \in \mathcal{F}_{h,c} \cup \mathcal{F}_{\Gamma}}
\int_{F}   \frac{\sigma \mu}{h_F} [\mathbf{u}_t ] \cdot [\mathbf{v}_t] \right ),   
\end{multline}
where on exterior facets the average and jump operators take the one-sided values. We set the penalty parameter for the tangential continuity to be $\sigma = 20$. The finite element discretization of the incompressible Stokes equations is then to find $(\mathbf{u}_h, p_h) \in \mathbf{V}_{h,g} \times Q_h$ with $g  = \frac{1}{|\Gamma_{\mathrm{LV}}|} u_{\mathrm{in}}$  such that 
\begin{subequations}
\begin{align}
  \mathcal{A}_h(\mathbf{u}_h, \mathbf{v}_h)
  + \sum_{F \in \mathcal{F}_{\rm AM-U}} \int_{F} R_{\rm CSF, 0} (\mathbf{u}_h \cdot \mathbf{n}) (\mathbf{v}_h \cdot \mathbf{n})
  - \int_{\Omega_{\rm CSF}} \nabla \cdot \mathbf{v}_h \, p_h  &= 0 \qquad \foralls \mathbf{v}_h \in \mathbf{V}_{h,0} \\ 
\int_{\Omega_{\rm CSF}} \nabla \cdot \mathbf{u}_h \, q_h  &= 0 \qquad \foralls q_h \in Q_h.
\end{align}
\end{subequations}
...
%%   %Q_h^{\rm PVS} &= \{ q_h \in C^0(\Lambda),  q_h \vert_{\Lambda^i} \in \mathrm{P}^k(\Lambda^i), q_h \vert_{\Gamma_{\rm Pia}} = p_{\rm CSF} \}
%% \end{align}
...
\item
  ...
  \begin{equation}\label{eq:pulsatility:bcs}
    \mathbf{u} = 0 \text{ on } \Gamma_{\rm Pia}; \quad 
    \mu \nabla \mathbf{u} \cdot \mathbf{n} - p \mathbf{n} = 0 \text{ on } \Gamma_{\rm SSAS}; \quad 
    \mathbf{u} = \frac{u^{\rm in}_{\rm LV} \cdot \mathbf{n}}{|\Gamma_{\rm LV}|} \text{ on } \Gamma_{\rm LV}; \quad
    \mathbf{u} = \frac{u^{\rm in}_{\rm AM} \cdot \mathbf{n}}{|\Gamma_{\rm skull}|} \text{ on } \Gamma_{\rm AM}.
\end{equation}
...
\item
  ...
  \begin{equation}
    ...
    \qquad \text{ and } \quad
    \nabla R_i \cdot \mathbf{n} = 0, \quad \text{ on } \partial \Omega_{\rm CSF} .
\end{equation}
\end{enumerate}
...
\begin{equation}
  \partial_t (\phi c) - \nabla \cdot (D \nabla (\phi c) ) + \nabla \cdot (\mathbf{u} c ) + \xi (\overline{c} - \hat c ) \delta_\Gamma
  %= f
   = S_{\text{0D} \leftrightarrow \text{3D}}
  \quad \quad \mathrm{in} \quad \Omega_{\rm CSF}, \Omega_{\rm{PAR}}.
  \label{eq:3d_pde}
\end{equation}
In~\eqref{eq:3d_pde}, $S_{\text{0D} \leftrightarrow \text{3D}}$ is a localized mass transfer term coupling the 3D domains to unresolved perivascular continuations, $\phi$ is the fluid volume fraction (also known as the porosity) defined in the parenchyma ($\phi \ll 1$ in $\Omega_{\rm PAR}$) and in the CSF spaces ($\phi = 1$ in $\Omega_{\rm CSF}$). In the parenchyma, $\phi$ represents the extracellular space volume fraction, and thus $c$ here generally represents the intrinsic (in contrast to the superficial) concentration~\cite{sykova2008diffusion}. Moreover, $D$ is the effective diffusion coefficient of the relevant solute in the respective media which takes different values over the CSF spaces and the parenchyma, depending on tortuosity~\cite{sykova2008diffusion} and dispersive effects; and $\mathbf{u}$ is a superficial convective velocity field representing the flow of CSF in $\Omega_{\rm CSF}$ and the (Darcy) flow of ISF in $\Omega_{\rm PAR}$.

 $\xi$ models a transfer or exchange parameter between the 3D domain ($\Omega_{\rm PAR}$ or $\Omega_{\rm CSF}$) and the perivascular networks $\Lambda_a, \Lambda_v$. To summarize,
\begin{equation*}
\phi =  \begin{cases}
...
D = \begin{cases}
  ...
  \quad \mathbf{u}  = \begin{cases}
  \mathbf{u}_{\rm{CSF}} & \mathrm{in} \; \Omega_{\rm{CSF}} \\ 
  \mathbf{u}_{\rm{PAR}} & \mathrm{in} \; \Omega_{\rm{PAR}} 
\end{cases}, \; 
\xi = \begin{cases}
...
%% \begin{subequations}
%% \begin{alignat}{2}
%% (-D \nabla (\phi c_{\rm{PAR}}) + \mathbf{u}_{\rm{PAR}} c_{\rm{PAR} }) \cdot \mathbf{n} &= - (- D (\phi \nabla c_{\rm{CSF}}) + \mathbf{u}_{\rm{CSF}} c_{\rm{CSF}} ) \cdot \mathbf{n} &&  \quad  \mathrm{on} \quad \Gamma_{\rm pia} \cup \Gamma_{\mathrm{LV}} ,  \\  
%% (-D \nabla ( c_{\rm{PAR}}) + \mathbf{u}_{\rm{PAR}} c_{\rm{PAR} }) \cdot \mathbf{n} & = \beta_{\rm{pia}} (c_{\rm PAR} - c_{\mathrm{CSF}}) &&  \quad  \mathrm{on} \quad \Gamma_{\rm pia} \cup \Gamma_{\mathrm{LV}} ,  
%% \end{alignat} 
%% \end{subequations}
\begin{subequations}
  \begin{alignat}{2}
  (-D \nabla (\phi c) + \mathbf{u} c)|_{\Omega_{\rm PAR}} \cdot \mathbf{n}
  &= - (-D \nabla (\phi c) + \mathbf{u} c)|_{\Omega_{\rm CSF}} \cdot \mathbf{n}
  &&  \quad  \mathrm{on} \quad \Gamma_{\rm pia} \cup \Gamma_{\mathrm{LV}} ,  \\  
  (- D \nabla (\phi  c) + \mathbf{u} c)|_{\Omega_{\rm PAR}} \cdot \mathbf{n} & = \beta_{\rm{pia}} (c_{\rm PAR} - c_{\mathrm{CSF}}) &&  \quad  \mathrm{on} \quad \Gamma_{\rm pia} \cup \Gamma_{\mathrm{LV}} ,  
\end{alignat} 
  \label{eq:3d_interface}%
\end{subequations}%
where $\mathbf{n} = \mathbf{n}_{\rm PAR}$ is the normal vector field defined
...
\begin{subequations}
\begin{alignat}{2}
(D \nabla (\phi c) - \mathbf{u} c ) \cdot \mathbf{n} &= g_{\mathrm{influx}} &&  \quad  \mathrm{on} \quad \Gamma_{\rm SSAS},  \\ 
% (-D \nabla (\phi c) + \mathbf{u} c ) \cdot \mathbf{n}_{\Gamma_{\rm SAS}} &= 0 &&  \quad  \mathrm{on} \quad \Gamma_{\rm SAS}^{\mathrm{out}}  \\ 
(-D \nabla (\phi c) + \mathbf{u} c ) \cdot \mathbf{n} & = \beta_{\rm exit} c  &&  \quad  \mathrm{on} \quad \Gamma_{\rm AM-U},  \\ 
(-D \nabla (\phi c) + \mathbf{u} c ) \cdot \mathbf{n} & = 0  &&  \quad  \mathrm{on} \quad \Gamma_{\rm AM-L} \cup \Gamma_{\rm SC}  .
\end{alignat} 
\label{eq:3d_bcs}
\end{subequations}
where again $\mathbf{n}$ denotes the outward-pointing boundary normal.
...
Here the function $n^i$ takes the values in $\{-1, 1\}$ and defines an orientation of vertices of edge $\Lambda^i$. Specifically, for an edge $\Lambda^i$ oriented from $y_{\rm in}$ to $y_{\rm out}$, we set
\begin{equation}
  n^i(y_{\rm in}) = -1, \quad n^i(y_{\rm out}) = 1.
\end{equation}
With this convention, at any network end node $y_k \in \mathcal{N}$ connected to an edge $\Lambda^i$, the normal pointing outward from the network is simply $n(y_k) = n^i(y_k)$.

Finally, to augment~\eqref{eq:1d_pde} at the network ends, we represent the unresolved, continuous extension of the arterial perivascular networks by coupling the arterial leaf nodes to a set of 0D compartments, similar to the approach in \cite{fritz20221d}. We denote this subset of arterial leaf nodes as $\mathcal{N}_{\rm leaf} \subset \mathcal{N}$. Root nodes and perivenous end nodes are excluded from this 0D coupling; at these nodes, a standard no-flux boundary condition is applied. For each arterial leaf node $y_k \in \mathcal{N}_{\rm leaf}$, let $c^0_k(t)$ denote the solute concentration in a 0D compartment of effective volume $V_k$. This compartment exchanges mass with the 3D domain over a local spherical neighborhood $B_k$ (with volume $|B_k|$) centered at $y_k$. The rate of change of mass in the 0D compartment is governed by advective and diffusive fluxes from the 1D network, alongside diffusive exchange with the 3D tissue:

\begin{equation}
  V_k \frac{d c^0_k}{dt} = q_k^+ \hat{c}(y_k) + q_k^- c^0_k + \alpha_k (\hat{c}(y_k) - c^0_k) + \beta_k \omega_k ( \bar{c}_{B_k} - c^0_k ) \quad \forall y_k \in \mathcal{N}_{\rm leaf},
  \label{eq:0d_ode}
\end{equation}
where $q_k = (A\hat{u})(y_k) n(y_k)$ is the volumetric fluid flow rate leaving the 1D network at $y_k$, split into outflow $q_k^+ = \max(q_k, 0)$ and inflow $q_k^- = \min(q_k, 0)$ components to implement upwinding, $\alpha_k$ is the 1D-0D diffusive conductance and $\beta_k$ is the nominal 0D-3D diffusive exchange rate. To account for boundary proximity where the neighborhood $B_k$ may partially extend outside the 3D domain, we scale this exchange by $\omega_k = |B_k \cap \Omega| / |B_k|$, which represents the volume fraction of the ball intersecting the computational mesh. Accordingly, $\bar{c}_{B_k}(t) = \frac{1}{|B_k \cap \Omega|} \int_{B_k \cap \Omega} c(x, t) \, dx$ is the average 3D concentration evaluated within this intersection.

To conserve mass, the total flux (advective and diffusive) exiting the 1D domain at $y_k$ must equal the total flux entering the 0D compartment from the 1D network. This yields the following Robin-type boundary condition on the 1D network:
\begin{equation}
  -\hat{D}A \partial_s \hat{c} = (q_k^- - \alpha_k)( c^0_k - \hat{c} ) \quad \mathrm{on} \quad y_k \in \mathcal{N}_{\rm leaf}.
  \label{eq:1d_bcs}
\end{equation}
We note that if advection is purely outward ($q_k > 0$, so $q_k^- = 0$), the boundary condition simplifies to $-\hat{D}A\partial_s\hat{c} = \alpha_k(\hat{c} - c^0_k)$, indicating that the diffusive flux at the boundary is driven purely by the concentration difference between the two compartments. Conversely, if $q_k < 0$ (flow enters 1D from 0D), the boundary condition balances the total diffusive flux with both the advective influx from the 0D compartment and the 1D-0D diffusive exchange. 

Finally, the diffusive exchange between the 0D compartments and the 3D domain acts via a localized volumetric source term $S_{\text{0D} \leftrightarrow \text{3D}}$ in~\eqref{eq:3d_pde}, defined using the indicator function $\chi_{B_k}$ for each ball $B_k$:
\begin{equation}
  S_{\text{0D} \leftrightarrow \text{3D}}(x, t) = \sum_{y_k \in \mathcal{N}_{\rm leaf}} \frac{\omega_k \beta_k}{|B_k \cap \Omega|} (c^0_k - \bar{c}_{B_k}) \chi_{B_k}(x).
  \label{eq:3d_source}
\end{equation}

To estimate the effective volume $V_k$ of the unresolved downstream perivascular network at each end node $y_k$, we model the vasculature as a branching tree. Assuming symmetric bifurcation and adherence to Murray's Law, the arterial radius at the $j$-th unresolved level scales as $r_{1,j} = R_1 2^{-j/3}$, and the outer PVS radius scales proportionally as $r_{2,j} = R_2 2^{-j/3}$. Assuming a constant length-to-arterial-radius ratio $\lambda = L_j / r_{1,j}$, the total perivascular volume of each level remains constant. Summing the segment volumes over $N$ unresolved levels down to the precapillary level yields the total downstream volume of the 0D compartment of each end node:
\begin{equation}
  V_k = A \lambda R_1 (N + 1).
  \label{eq:effective_volume}
\end{equation}

Similarly, the 0D-3D diffusive exchange rate $\beta_k$ is governed by the total outer surface area $S_k$ of the unresolved perivascular network interfacing with the surrounding 3D tissue. Unlike the geometrically constant perivascular volume, the outer surface area increases with each subsequent branching level. Applying the same branching assumptions, the surface area of a single PVS segment at the $j$-th unresolved level is $2 \pi r_{2,j} L_j = 2 \pi \lambda r_{1,j} r_{2,j}$. Multiplying by the $2^j$ vessels in that level and substituting $r_{1,j} = R_1 2^{-j/3}$ and $r_{2,j} = R_2 2^{-j/3}$, the total tissue-facing surface area over $N$ levels is given by:
\begin{equation}
  S_k = 2 \pi \lambda R_1 R_2 \sum_{j=0}^{N} 2^{j/3}.
  \label{eq:surface_area}
\end{equation}
The effective 0D-3D diffusive exchange rate is then defined as $\beta_k = \xi_{\text{EF}} S_k$.

Finally, the 1D-0D diffusive conductance $\alpha_k$ governs the exchange of solutes across the interface at $y_k$. Based on Fick's law, this conductance is proportional to the perivascular cross-sectional area $A$ and inversely proportional to a characteristic diffusion length $L_c$. We define the characteristic length $L_c$ as the length of the first unresolved level, such that $L_c = \lambda R_1$. With the diffusion coefficient $D$, the conductance evaluates to:
\begin{equation}
  \alpha_k = \frac{D A}{L_c} = \frac{D A}{\lambda R_1}.
  \label{eq:conductance}
\end{equation}

The morphological parameters $\lambda$ and $N$ are chosen based on established quantitative measurements of the cerebral microvasculature \cite{cassot2006novel, lorthois2014tortuosity}. For a typical leaf node with radius $R_1 = 0.5\,$ mm, we set the length-to-radius ratio to $\lambda = 16$ \cite{lorthois2014tortuosity}. Resolving the network down to $N = 15$ unresolved branching levels scales the terminal vessels to approximately $15 \, \mu$m, matching the characteristic dimension of a precapillary arteriole. Finally, we set the radius of all balls $B_k$ to $10\,$mm.

For the remaining network end nodes, comprising the arterial root nodes and all perivenous end nodes, we impose a homogeneous no-flux boundary condition. Ensuring that the sum of the outward diffusive and advective fluxes is zero yields:
\begin{equation}
  \hat{D}A \partial_s \hat{c} - (A \hat{u} n(y_k)) \hat{c} = 0 \quad \mathrm{for} \quad y_k \in \mathcal{N} \setminus \mathcal{N}_{\rm leaf}.
  \label{eq:1d_noflux}
\end{equation}
...
\begin{equation}
  a_h^{\mathrm{up}}(c, v)
  = - \sum_{E \in \mathcal{T}} \int_E c\mathbf{u} \cdot \nabla v
  + \sum_{F \in \mathcal{F}_i } \int_F \{ \mathbf{u}  c\} \cdot \mathbf{n}_F [v] +
  \sum_{F \in \mathcal{F}_i} \int_F \frac{|\mathbf{u} \cdot \mathbf{n}_F|}{2} [c] [v] .
\end{equation}
...
\begin{align} \label{eq:3d_dg_disc}
\begin{split}
  ...
  - \sum_{y_k \in \mathcal{N}_{\rm leaf}} \frac{\omega_k \beta_k}{|B_k \cap \Omega|} \left(c_{k}^{0,n} - \bar{c}_{B_k}^n\right) \int_{B_k \cap \Omega} v
  =
  %\int_{\Omega} f^n v +
...
\end{split}
\end{align}
where $\bar{c}_{B_k}^n$ is the discrete spatial average of the 3D concentration within the local neighborhood intersection, defined as:
\begin{equation}
\bar{c}_{B_k}^n = \frac{1}{|B_k \cap \Omega|} \int_{B_k \cap \Omega} c_h^n .
\end{equation}
...
\end{equation}
where $\mathrm{P}^1(\Lambda^i)$ is the space of linear polynomials on each $\Lambda^i$. The discrete formulation then reads: for $n = 1,2,\ldots, $ find $\hat c_h^n \in \hat V_h$ such that for all $\hat v \in \hat V_h$:


\begin{align}\label{eq:1d_dg_disc}
\begin{split}
  \int_{\Lambda}  \frac{1}{\tau}(A  \hat c_h^n - A \hat c_h^{n-1}) \hat v  &+ \int_{\Lambda} \hat D A \partial_s \hat c_h^n  \partial_s \hat v - \int_{\Lambda} A \hat u \hat c_h^n \partial_s \hat v   +  \int_{\Lambda} \xi P (\hat c_h^n - \overline{c_h^n}) \hat v \\
&+ \sum_{y_k \in \mathcal{N}_{\rm leaf}} \left( q_k^+ \hat{c}_h^n(y_k) + q_k^- c_{k}^{0,n} + \alpha_k (\hat{c}_h^n(y_k) - c_{k}^{0,n}) \right) \hat{v}(y_k) + a_h^{\mathrm{stab}}(\hat u; \hat c_h^n, \hat v) = 0.  
\end{split}
\end{align}

 In the above, if $\hat u$ is non--zero on $\Lambda^i$, then the artificial diffusion $a_h^{\mathrm{stab}}$ stabilization term is nonzero  and is given below, for more details see \cite[Section 12.6]{quarteroni2009numerical}. 
 \begin{align}
    a_h^{\mathrm{stab}}(\hat u; \hat c, \hat v) =  \frac{1}{\|\hat u\|_{L^2(\Lambda^i)}} 
 h_{\Lambda^i} \int_{\Lambda^i} A (\hat u \partial_s \hat c  ) (\hat u \partial_s \hat v),  
 \end{align}
...
For the 0D compartments at the arterial leaf nodes $y_k \in \mathcal{N}_{\rm leaf}$, the discrete weak equations are advanced in time using an implicit Euler scheme: for $n = 1, 2, \dots$, find $c_{k}^{0,n} \in \mathbb{R}$ such that
\begin{equation} \label{eq:0d_disc}
\left( V_k \frac{c_{k}^{0,n} - c_{k}^{0,n-1}}{\tau} \right) v_k^0 = \left( q_k^+ \hat{c}_h^n(y_k) + q_k^- c_{k}^{0,n} + \alpha_k (\hat{c}_h^n(y_k) - c_{k}^{0,n}) + \beta_k \omega_k ( \bar{c}_{B_k}^n - c_{k}^{0,n} ) \right) v_k^0,
\end{equation}
holds for all $v_k^0 \in \mathbb{R}$ associated with each individual leaf node $y_k \in \mathcal{N}_{\rm leaf}$.

Summary. The discretization for \eqref{eq:3d_pde}, \eqref{eq:1d_pde}, and \eqref{eq:0d_ode} complemented with boundary conditions (\eqref{eq:3d_bcs} and \eqref{eq:1d_bcs}), interface and bifurcation conditions (\eqref{eq:3d_interface}, and \eqref{eq:conc_continuity}), and initial conditions $c_h^0 = 0$, $\hat c_h^0 = 0$, and $c_{k}^{0,0} = 0$ is the following. For $n = 1,2, \ldots $, find $c_h^n \in V_h$, $\hat c^n_h \in \hat V_h$, and $c_{k}^{0,n} \in \mathbb{R}$ for each $y_k \in \mathcal{N}_{\rm leaf}$ such that the coupled discrete equations \eqref{eq:3d_dg_disc}, \eqref{eq:1d_dg_disc}, and \eqref{eq:0d_disc} hold for all $v \in V_h$, for all $\hat v \in \hat V_h$, and for all $v_k^0 \in \mathbb{R}$ associated with each $y_k \in \mathcal{N}_{\rm leaf}$.
...
\begin{figure}
    ...
    Source data for all panels are provided as Source data files.
    }
...
\subsection{Effect of 0D-compartments at arterial leaf nodes}

To evaluate whether the 0D downstream compartments are necessary to capture perivascular transport dynamics, we tested an alternative formulation in which they are omitted. Instead, we applied a strict no-flux boundary condition at the arterial tips and artificially increased the lateral diffusive permeability at the leaf network nodes (linearly increasing from the second-to-last to the last node). We evaluated this approach using the vasomotion model, as this high advective flow regime is most likely to produce artificial tracer build up at the leaf nodes. Testing permeability multipliers of 1, 10, 100, and 1000, we found that macroscopic transport metrics are robust: the total tracer content across compartments (Figure~\ref{fig:xi_multiplier_comparison}A) and the time of first tracer arrival (FTA) along the main branches (Figure~\ref{fig:xi_multiplier_comparison}B) show almost no sensitivity to the multiplier value. However, this alternative model produces severe local artifacts. The combination of strong advective flow and a no-flux boundary results in an artificial tracer build-up at the end nodes. With standard permeability (a multiplier of 1), tip concentrations reach non-physical values of up to $1200\,$mmol/l (Figure~\ref{fig:xi_multiplier_comparison}C). While artificially increasing the permeability mitigates this bottleneck, even a 1000-fold increase yields unrealistic local concentration spikes of up to $120\,$mmol/l. Thus, while the alternative boundary treatment is robust for globally averaged quantities and upstream transport dynamics, it suffers from substantial local artifacts at the arterial tips.

\begin{figure}
    \centering
    \includegraphics[width=0.99\linewidth]{xi_multiplier_comparison.pdf}
    \caption{
    A) Total tracer content in the CSF, parenchyma, and arterial and venous PVS for variations of the high PVS flow model with a permeability multipliers of 1, 10, 100, and 1000 at the PVS network leaf nodes;
    B) First-time arrival of tracer along branches of the arterial tree for the model variants with permeability multiplier;
    C) Tracer concentrations at the individual arterial leaf nodes for the model variants with permeability multiplier.
    Source data for all panels are provided as Source data files.
    }
    \label{fig:xi_multiplier_comparison}
\end{figure}


To address this, we compared the 100-fold permeability multiplier model against our 0D compartment model. In terms of total compartmental tracer content, the overall macroscopic dynamics remain similar between the two approaches (Figure~\ref{fig:0D_xi_multiplier_comparisonA}A and B). However, in the model with 0D compartments, $14\%$ of the total tracer remains within the 0D compartments after 24 hours, accompanied by to a corresponding reduction in total parenchymal tracer ($31\%$ compared to $47\%$ in the model lacking 0D compartments). Crucially, the 0D compartments successfully resolve the leaf node tracer accumulation: tracer concentrations at the leaf nodes and within the 0D compartments remain physiologically reasonable (below $40\,$mmol/l and $22\,$mmol/l, respectively), in contrast to the spikes of up to $150\,$mmol/l observed in the model without 0D compartments (Figure~\ref{fig:0D_xi_multiplier_comparisonA}C--E). Furthermore, upstream FTAs remain unaffected by the choice of 0D model (Figure~\ref{fig:0D_xi_multiplier_comparisonA}F).

Finally, the overall spatial tracer distribution patterns remain largely consistent between the two models (Figure~\ref{fig:0D_xi_multiplier_comparisonB}. Interestingly, despite a lower total tracer mass in the parenchyma, the 0D model predicts slightly deeper tracer penetration into the tissue (visible, for instance, in the axial views of the left hemisphere at 9, 12, and 24 hours). This phenomenon occurs because the tracer entering the 0D compartments is rapidly exchanged into all tissue within the localized integration ball $B_k$, facilitating a deeper tracer penetration.

\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{0D_xi_multiplier_comparisonA.png}
    \caption{
    Total tracer content in the CSF, parenchyma, and arterial and venous PVS for the high PVS flow model with 0D compartments (A);
    and the same model without 0D compartments, but a 100x permeability multiplier at PVS leaf nodes (B);
    C) Tracer concentrations at the individual 0D compartments for the model with 0D compartments;
    D) Tracer concentrations at the individual arterial leaf nodes for the model with 0D compartments;
    E) Tracer concentrations at the individual arterial leaf nodes for the model without 0D compartments, but 100x permeability multiplier.
    F) First-time arrival of tracer along branches of the arterial tree for the both models;
    Source data for all panels are provided as Source data files.
    }
    \label{fig:0D_xi_multiplier_comparisonA}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{0D_xi_multiplier_comparisonB.png}
    \caption{
    A+B) sagittal, coronal and axial view of in-silico tracer concentrations overlayed on T1-weighted MR image after 9, 12, and 24\,h (low concentrations transparent, pial surface in pink, arachnoid membrane in cyan, and arteries in dark red) for the model with 0D compartments (A) and without (B);
    C) the concentration difference between the two models.
    }
    \label{fig:0D_xi_multiplier_comparisonB}
\end{figure}
...
distribution and clearance against contrast-enhanced MRI, we here compare
...
respiratory-driven CSF flow estimates peak at 19.8 cm/s and 4.8 cm/s in the caudal direction (\Cref{fig:flow_plots}), respectively, which are higher than phase-contrast MRI measurements of cardiac and respiratory CSF flow
...
distribution patterns. Our spatially-varying estimates of the dispersion
...
\begin{figure}
    ...
    Source data for all histograms are provided as Source data files.
    }
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{suppl_flow_figures.pdf}
    \caption{
      A) Peak CSF flow induced by cardiac pulsatility; 
      B) Peak CSF flow induced by respiratory pulsatility; 
      C) Histograms of the cardiac-induced CSF velocities in the SAS and third and fourth ventricle (V3 \& V4) in terms of the relative frequency of the mean flow speed in each computational cell weighted by its volume;
      D) Histograms as in C), but for respiratory-induced flow.
      Source data for panels C) and D) are provided as Source data files.
      }
    \label{fig:flow_plots}
\end{figure}
...
\renewcommand{\refname}{Supplementary References} % Nature Comms requests
