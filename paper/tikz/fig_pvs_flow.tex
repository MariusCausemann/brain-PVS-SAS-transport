...
\caption{Estimating the perivascular flow induced by vascular wall motion. (\textbf{A}) For a vascular network with vessels
    ...
    bifurcating and directed tree while preserving path lengths and
    averaging radii (\textbf{C}) which is then used to compute the net
    flow induced by the peristaltic wave in each branch of the minimal
    subnetworks (\textbf{D}). To conserve mass, we distribute the
    computed flow onto the original segments
    (\textbf{E}).}
