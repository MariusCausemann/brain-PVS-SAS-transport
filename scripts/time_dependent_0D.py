...
def run_simulation(configfile: str):
    ...
    vein_shape = xii.Circle(radius=vein_radii, degree=40, quad_scheme="midpoint")    
    ...
    u, pa, pv, pa0 = map(TrialFunction, W)
    v, qa, qv, qa0 = map(TestFunction, W)
   ...
   # downstream reservoir terms:
    na = FacetNormal(artery)
    artery_tips = MeshFunction("size_t", artery, 0, 0)
    ...
    tips.save(results_dir + f'{modelname}_tips.vtk')
