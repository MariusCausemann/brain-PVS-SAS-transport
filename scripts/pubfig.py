...
class RasterImage:
    def __init__(self, file: Union[Path, str], img_size: ElemSize, crop=None):
        ...
        if crop is not None:
            ...
            with Image.open(file) as img:
                ImageOps.crop(img, crop).save(tmpfile)
            file = tmpfile
        self.file: Path = Path(file)
...
def composite(
    ...
    if fig_spec.generate_image != ImageType.none:
        ...
        height_inch = fig_spec.figure_size.units.to_inches(fig_spec.figure_size.height)
        output_height = int(fig_spec.image_dpi * height_inch)
        image_name = f"{basename}.png"
        cmd = f"cairosvg {svg_path} -o {image_name} --output-height {output_height} --background '#FFFFFF'"
        print(cmd)
        _run(cmd)
