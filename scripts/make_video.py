...
def make_movie(modelname:str):
    ...
    dt , T= config["dt"]*config["output_frequency"], config["T"]
    ...
    for t in tqdm(times[:-1]):
        img[f"c_{int(t + dt/2)}"] = 0.5 *(img[f"c_{t}"] + img[f"c_{t + dt}"])
        arteries[f"c_{int(t + dt/2)}"] = 0.5 *(arteries[f"c_{t}"] + arteries[f"c_{t + dt}"])
